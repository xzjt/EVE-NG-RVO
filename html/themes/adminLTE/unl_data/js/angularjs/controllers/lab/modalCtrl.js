function ModalCtrl($scope, $uibModal, $log, $rootScope, $http, $window) {
    ...
    $scope.modalActions = {
        'addConn': {
            'path': '/themes/adminLTE/unl_data/pages/modals/addConn.html',
            'controller': 'AddConnModalCtrl'
        },
        'addNode': {
            'path': '/themes/adminLTE/unl_data/pages/modals/addNode.html',
            'controller': 'AddNodeModalCtrl'
        },
        'addText': {
            'path': '/themes/adminLTE/unl_data/pages/modals/addText.html',
            'controller': 'AddTextModalCtrl'
        },
        'addShape': {
            'path': '/themes/adminLTE/unl_data/pages/modals/addShape.html',
            'controller': 'AddShapeModalCtrl'
        },
        'addNet': {
            'path': '/themes/adminLTE/unl_data/pages/modals/addNet.html',
            'controller': 'AddNetModalCtrl'
        },
        'editNode': {
            'path': '/themes/adminLTE/unl_data/pages/modals/editNode.html',
            'controller': 'editNodeModalCtrl'
        },
        'editNet': {
            'path': '/themes/adminLTE/unl_data/pages/modals/editNet.html',
            'controller': 'editNetModalCtrl'
        },
        'editLab': {
            'path': '/themes/adminLTE/unl_data/pages/modals/editLab.html',
            'controller': 'editLabModalCtrl'
        },
        'nodeList': {
            'path': '/themes/adminLTE/unl_data/pages/modals/nodeList.html',
            'controller': 'nodeListModalCtrl'
        },
        'netList': {
            'path': '/themes/adminLTE/unl_data/pages/modals/netList.html',
            'controller': 'netListModalCtrl'
        },
        'sysStat': {
            'path': '/themes/adminLTE/unl_data/pages/modals/sysStat.html',
            'controller': 'sysStatModalCtrl'
        },
        'startUpConfig': {
            'path': '/themes/adminLTE/unl_data/pages/modals/startUpConfig.html',
            'controller': 'startUpConfigModalCtrl'
        },
        'labDetails': {
            'path': '/themes/adminLTE/unl_data/pages/modals/labDetails.html',
            'controller': 'labDetailsModalCtrl'
        },
        'configObject': {
            'path': '/themes/adminLTE/unl_data/pages/modals/configObject.html',
            'controller': 'configObjectModalCtrl'
        },
        'default': {
            ...
            resolve: {
                data: function() {
                    ...
                    switch (action) {
                        case 'addConn':
                            return {
                                'src': $scope.addConnSrc, 'dst': $scope.addConnDst, 'path': $rootScope.lab, 'allNet': $rootScope.allNetworks
                            };
                            break;
                        case 'addNode':
                            return {
                                'path': $rootScope.lab, 'object': $scope.addNewObject
                            };
                            break;
                        case 'addNet':
                            return {
                        ...
                        case 'editNode':
                            return {
                                'path': $rootScope.lab, 'id': $scope.tempEldata.id
                            };
                            break;
                        case 'editNet':
                            return {
        ...
        switch (action) {
                    ...
                    if (result.result) {
                        if (result.connType == 'NoNe') {
                            ...
                            jsPlumb.connect({
                                ...
                                paintStyle: {
                                    strokeStyle: (type != 'serial') ? '#1e8151' : '#ffd11a',
                                    lineWidth: 2,
                                    outlineColor: "white",
                                    outlineWidth: 1
                                },
                ...
                modalInstance.result.then(function(result) {
                    ...
                    if (result.result) {
                        ...
                        var elDIV = '<div id="networkID_' + result.data.id + '" class="w element-menu network" style="left: ' + result.data.left + 'px; top: ' + result.data.top + 'px;" context-menu="netContext">' +
                            ...
                            '</div>';
                        $('#canvas').append(elDIV);
                        $('#networkID_' + result.data.id).ready(function() {
                            jsPlumbNodeInit($('#networkID_' + result.data.id)) /*console.log('networkID_'+networkObject['id']+' ready');*/
                        });
            ...
            case 'editNet':
                modalInstance.result.then(function(result) {
                    ...
                    $('div#networkID_' + result.id).offset({
                        left: result.left,
                        top: result.top
                    })
    ...
    $http.get('/api/list/templates/').then(
        function successCallback(response) {
            ...
            for (var key in response.data.data) {
                $scope.tempList[index] = {
                    'brname': key,
                    'fullname': response.data.data[key]
                }
                index++
            }
        },
...
function nodeListModalCtrl($scope, $uibModalInstance, $http, data, $rootScope, $state) {
                ...
                .then(
                    function successCallback(response) {
                        ...
                        for (var key in $scope.templateList[template].icon.list) {
                            $scope.templateList[template].icon.array[iconIndex] = {
                                'fullname': key,
                                'brname': $scope.templateList[template].icon.list[key]
                            }
                            iconIndex++
                        }
                        var imageIndex = 0;
    ...
    $scope.deleteNode = function(id, name) {
        ...
        if ($scope.nodeList[id].count > 0) {
            $scope.anychanges = true;
        }
        $http({
            method: 'DELETE',
            url: '/api/labs' + $scope.path + '/nodes/' + id
        }).then(
            function successCallback(response) {
                //console.log(response)
...
function netListModalCtrl($scope, $uibModalInstance, $http, data, $rootScope, $state) {
    ...
    $scope.deleteNet = function(id, name) {
        ...
        $http({
            method: 'DELETE',
            url: '/api/labs' + $scope.path + '/networks/' + id
        }).then(
            function successCallback(response) {
                // console.log(response)
