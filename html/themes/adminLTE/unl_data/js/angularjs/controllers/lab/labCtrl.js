function labController($scope, $http, $location, $uibModal, $rootScope, $q, $log, $compile, $timeout, $window, $timeout, $state) {
    ...
    $scope.mainFieldClick = function($event, src) {
        ...
        if (src == undefined) {
            $scope.addNewObject = {};
            $scope.addNewObject = $event
        } else {
            $scope.addNewObject = {};
            $scope.addNewObject.pageX = $('#' + src).offset().left;
            $scope.addNewObject.pageY = $('#' + src).offset().top;
        }
    ...
    $scope.setAllStartupExport = function() {
        ...
        $(".free-selected").each(function() {
            ...
            id = id.replace("nodeID_", "");
            var node = $scope.node[id];
            $http.put('/api/labs' + $rootScope.lab + '/nodes/' + id, {
                    'config': 1
                })
                .then(
                    function successCallback(response) {
        ...
        if (!h_flague) {
            for (i in $scope.node) {
                $http.put('/api/labs' + $rootScope.lab + '/nodes/' + i, {
                        'config': 1
                    })
                    .then(
                        function successCallback(response) {
    ...
    $scope.deleteAllStartupConfig = function() {
        ...
        $(".free-selected").each(function() {
            ...
            id = id.replace("nodeID_", "");
            var node = $scope.node[id];
            $http.put('/api/labs' + $rootScope.lab + '/configs/' + id, {
                    'data': ""
                })
                .then(
                    function successCallback(response) {
        ...
        if (!h_flague) {
            for (i in $scope.node) {
                $http.put('/api/labs' + $rootScope.lab + '/configs/' + i, {
                        'data': ""
                    })
                    .then(
                        function successCallback(response) {
    ...
    $scope.openTextConsole = function(node, $event) {
        if (!$scope.text[node].upstatus) {
            $event.preventDefault();
            console.log('Text down console locked')
        }
        if ($scope.textDraggingFlag) {
            $event.preventDefault();
            console.log('Text draged console locked')
        }
        $scope.textClickDown = false;
        $scope.textDraggingFlag = false;
    }
    ...
    $scope.openShapeConsole = function(node, $event) {
        if (!$scope.shape[node].upstatus) {
            $event.preventDefault();
            console.log('Shape down console locked')
        }
        if ($scope.shapeDraggingFlag) {
            $event.preventDefault();
            console.log('Shape draged console locked')
        }
        $scope.shapeClickDown = false;
        $scope.shapeDraggingFlag = false;
    }
    ...
    $scope.newConnModal = function(conn) {
        if ($scope.ready) {
            ...
            if (src.eveID != '') {
                if ($scope.node[src.eveID].status != 0) {
                    toastr["error"]("Stop nodes which you want to connect", "Error");
                    return;
                }
            }
            if (dst.eveID != '') {
                if ($scope.node[dst.eveID].status != 0) {
                    toastr["error"]("Stop nodes which you want to connect", "Error");
                    return;
                }
            }
        ...
        if (ifs.type == 'ethernet') {
            if (src.type == 'node' && dst.type == 'node') {
                ...
                $scope.getIntrfInfo(src.id).then(function(something) {
                    ...
                    for (var key in something.ethernet) {
                        if (something.ethernet[key].name == ifs.interfSrc) {
                            something.ethernet[key].network_id = "";
