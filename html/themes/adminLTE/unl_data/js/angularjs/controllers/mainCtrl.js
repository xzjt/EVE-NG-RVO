function mainController($scope, $http, $location, $window, $uibModal, $log, $rootScope, FileUploader, focus) {
    ...
    $scope.elementToggleFun = function(thatCreate) {
        $scope.hideAllEdit()
        if (!$scope.newElementToggle) {
            $scope.newElementToggle = true;
            focus('foCreate');
            $scope.thatCreate = thatCreate;
            return;
        }
        if ($scope.thatCreate == thatCreate && $scope.newElementToggle) {
            $scope.newElementToggle = false;
        }
    ...
    $scope.deleteElement = function(elementName, thatis, hide) {
        ...
        if (thatis == 'Folder') {
            if (tempVal)
                if (!confirm('Are you sure you want to delete this folder: ' + elementName)) return;
    ...
    $scope.deleteALLElement = function() {
        ...
        if (ObjectLength(folderArray) == 0 && ObjectLength(fileArray) == 0) {
            toastr["warning"]("Please select items to delete", "Warning");
            return;
        }
        var folderCount = 1;
        var fileCount = 1;
        ...
        if (ObjectLength(folderArray) > 0)
            for (var foldername in folderArray) {
                if (folderCount !== ObjectLength(folderArray) || ObjectLength(fileArray) != 0) {
                    commaChar = ',';
                } else commaChar = '';
        ...
        if (ObjectLength(fileArray) > 0)
            for (var filename in fileArray) {
                if (fileCount !== ObjectLength(fileArray)) commaChar = ',';
                else commaChar = '';
    ...
    $scope.editElementShow = function() {
        ...
        if (trueCheckbox == 0) {
            toastr["warning"]("Please select item to rename", "Warning");
            return;
        }
        if (trueCheckbox > 1) {
            toastr["warning"]("You can rename only 1 item", "Warning");
            return;
        }
    ...
    $scope.editElementApply = function(item) {
        ...
        if (itemType == 'Fo_') {
            ...
            $http({
                    ...
                    data: {
                        "path": tempPath + $scope.fileManagerItem[itemType + item.oldvalue].value
                    }
                })
                .then(
                    function successCallback(response) {
            ...
            $http({
                    method: 'PUT',
                    url: '/api/labs' + tempPath + $scope.fileManagerItem[itemType + item.oldvalue].oldvalue + '.unl',
                    data: {
                        "name": $scope.fileManagerItem[itemType + item.oldvalue].value
                    }
                })
                .then(
                    function successCallback(response) {
    ...
    $scope.exportFiles = function() {
        ...
        if (ObjectLength(fileExportArray) == 0) {
            toastr["warning"]("Please select items to export", "Warning");
            return;
        }
    ...
    $scope.moveto = function() {
        ...
        if (ObjectLength($scope.folderArrayToMove) == 0 && ObjectLength($scope.fileArrayToMove) == 0) {
            toastr["warning"]("Please select items to move", "Warning");
            return;
        }
