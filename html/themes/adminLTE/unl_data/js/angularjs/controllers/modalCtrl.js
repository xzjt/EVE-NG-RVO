function ModalCtrl($scope, $uibModal, $log) {
    ...
    $scope.modalActions = {
        'addfile': {
            'path': '/themes/adminLTE/unl_data/pages/modals/addfile.html',
            'controller': 'AddElModalCtrl'
        },
        'editfile': {
            'path': '/themes/adminLTE/unl_data/pages/modals/editfile.html',
            'controller': 'EditElModalCtrl'
        },
        'editLab': {
            'path': '/themes/adminLTE/unl_data/pages/modals/editLab.html',
            'controller': 'EditElModalCtrl'
        },
        'adduser': {
            'path': '/themes/adminLTE/unl_data/pages/modals/adduser.html',
            'controller': 'AddUserModalCtrl'
        },
        'edituser': {
            'path': '/themes/adminLTE/unl_data/pages/modals/edituser.html',
            'controller': 'EditUserModalCtrl'
        },
        'moveto': {
            'path': '/themes/adminLTE/unl_data/pages/modals/moveto.html',
            'controller': 'MoveToModalCtrl'
        },
        'default': {
            'path': '/themes/adminLTE/unl_data/pages/modals/wtf.html',
            ...
            resolve: {
                data: function() {
                    switch (action) {
                        case 'addfile':
                            return {
                                'name': $scope.newElementName, 'path': $scope.path
                            };
                            break;
                        case 'editfile':
                            $scope.labInfo.fullPathToFile = $scope.fullPathToFile;
                            return {
                                'info': $scope.labInfo, 'path': $scope.path
                            };
                            break;
                        case 'editLab':
                            $scope.labInfo.fullPathToFile = $scope.fullPathToFile;
                        ...
                        case 'adduser':
                            return {
                                'currentUserData': $scope.userdata
                            };
                            break;
                        case 'edituser':
                            return {
                                'username': $scope.edituser
                            };
                            break;
                        case 'moveto':
                            return {
                                'foldersArray': $scope.folderArrayToMove, 'filesArray': $scope.fileArrayToMove, 'path': $scope.path
                            };
                            break;
                        default:
                            return {
                                'wtf': $scope.newElementName, 'path': $scope.path
                            };
                    }
                }
            }
        ...
        $http({
            ...
            .then(
                function successCallback(response) {
                    ...
                    */
                    //alert("Lab Create Successfully!");
                    var lab_name = $scope.newdata.name
                    //alert("Lab " + lab_name + " has been created successfully!");
                    location.reload();
                },
...
function AddUserModalCtrl($scope, $uibModalInstance, $http, data) {
    ...
    $scope.addNewUser = function() {
        ...
        if ($scope.passwdConfirm != $scope.passwd) {
            $scope.errorClass = 'has-error passwdConfirm';
            $scope.errorMessage = "Password doesn't match";
        }
        if ($scope.passwdConfirm == '') {
            $scope.errorClass = 'has-error passwdConfirm';
            $scope.errorMessage = "Password can't be empty!";
        }
        if ($scope.passwd == '') {
            $scope.errorClass = 'has-error passwd';
            $scope.errorMessage = "Password can't be empty!";
        }
        if ($scope.username == '') {
            $scope.errorClass = 'has-error username';
            $scope.errorMessage = "Username can't be empty!";
        }
        if ($scope.passwd == 'whereismypassword?') {
            $scope.passwd = '';
        }
        if ($scope.errorClass != '') {
            return;
        ...
        $http.get('/api/users/').then(function(response) {
            ...
            if ($scope.podError) {
                toastr["error"]("Please set unique POD value", "Error");
                return;
            }
...
function MoveToModalCtrl($scope, $uibModalInstance, data, $http, $location, $interval) {
    ...
    $scope.fastSearch = function(pathInput) {
        ...
        if (pathInput == "" || pathInput.search(re) == -1) {
            $scope.openDropdown = "";
            return;
        }
    ...
    $scope.move = function() {
        ...
        $scope.errorMessage = "";
        if ($scope.newpath == "") {
            $scope.errorMessage = "New path can't be empty";
            return;
        }
        if ($scope.newpath.search(re) == -1 && $scope.newpath != "/") {
            $scope.errorMessage = "Unknown path format, be sure that you added '/' to the end";
            return;
        }
        if ($scope.pathForTest == $scope.newpath) {
            $scope.errorMessage = "Path can't be the same";
            return;
        }

        for (i = 0; i < $scope.folderdata.length; i++) {
            if ($scope.pathForTest + $scope.folderdata[i] + '/' == $scope.newpath) {
                $scope.errorMessage = "You can't select this directory";
                return;
            }
        ...
        $http.get('/api/folders' + $scope.newpath.replace(/\/$/, '')).then(
            ...
            function errorCallback(response) {
                ...
                if (response.status == 404 && response.statusText == 'Not Found') {
                    $scope.errorMessage = "You set incorrect path. Folder no found!";
                    $scope.folderfound = false;
                    return;
                }
                //console.log("Unknown Error. Why did API doesn't respond?"); $location.path("/login");
            }
            ...
            if ($scope.folderfound) {
                ...
                if ($scope.folderdata.length > 0)
                    for (fo = 0; fo < $scope.folderdata.length; fo++) {
                        ...
                        $http({
                                ...
                                data: {
                                    "path": $scope.newpath + $scope.folderdata[fo]
                                }
                            })
                            .then(
                                function successCallback(response) {
                                ...
                                function errorCallback(response) {
                                    ...
                                    if (response.data.message == "Destination folder already exists (60046).") {
                                        folderTester--;
                                        toastr["error"]("Destination folder already exists", "Error");
                                    }
                                    console.log("Unknown Error. Why did API doesn't respond?");
                                    //$location.path("/login");
                                }
                ...
                if ($scope.filedata.length > 0)
                    for (fi = 0; fi < $scope.filedata.length; fi++) {
                        ...
                        $http({
                                method: 'PUT',
                                url: '/api/labs' + $scope.pathForTest + $scope.filedata[fi] + '.unl' + '/move',
                                data: {
                                    "path": tempPathNew
                                }
                            })
                            .then(
                                ...
                                function errorCallback(response) {
                                    console.log(response)
                                    if (response.data.message == "Lab already exists (60016).") {
                                        fileTester--;
                                        toastr["error"]("Lab already exists", "Error");
                                    }
                                    if (response.status == 412 && response.data.status == "unauthorized") {
                                        console.log("Unauthorized user.")
                                        $uibModalInstance.dismiss('cancel');
