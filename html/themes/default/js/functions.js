...
function addModalWide(title, body, footer, property) {
    ...
    if (title.toUpperCase() == "STARTUP-CONFIGS" || title.toUpperCase() == "CONFIGURED NODES" || title.toUpperCase() == "CONFIGURED TEXT OBJECTS" || title.toUpperCase() == "CONFIGURED NETWORKS" || title.toUpperCase() == "CONFIGURED NODES" || title.toUpperCase() == "STATUS" || title.toUpperCase() == "PICTURES") {
...
function deleteSingleNetworks() {
    ...
    $.when(getNetworks()).then(function(networks) {
        var deleted = [];
...
function deletePicture(lab_file, picture_id, cb) {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data['status'] == 'success') {
                // Fetching ok
                $('.picture' + picture_id).fadeOut(300,
                function() {
                    $(this).remove();
                });
                deferred.resolve(data);
...
function printFormNode(action, values, fromNodeList) {
    ...
    if ($.cookie("full-list") == undefined) {
        $.cookie("full-list", 0)
    }
    var b = $.cookie("full-list");
    logger(2, "node list visibility (cookie) : " + $.cookie("full-list"));
    $.when(getTemplates(null)).done(function(templates) {
        var html = '';
        html += '<form id="form-node-' + action + '" ><div class="form-group col-sm-12"><label class="control-label">' + MESSAGES[84] + '</label>'
        if (action == 'add') {
            html += '<label style="float:right;margin-left:6px">' + MESSAGES[233] + '</label><input id="full-list" type="checkbox" title="' + MESSAGES[234] + '" name="full-list" value="' + b + '" ' + (b == 1 ? "checked": "") + ' style="float:right"></div>'
        }
        html += '<div><select id="form-node-template" class="selectpicker form-control" name="node[template]" data-live-search="true" data-size="auto" data-style="selectpicker-button">' + '<option value="">' + MESSAGES[102] + '</option>';
        $.each(templates,
        function(key, value) {
        ...
        $('#form-node-template').change(function(e2) {
            ...
            if (template != '') {
                ...
                $.when(getTemplates(template), getNodes(id)).done(function(template_values, node_values) {
                    ...
                    if (action == 'add') {
                        // If action == add -> print the nework count input
                        html_data += '<div class="form-group col-sm-5"><label class=" control-label">' + MESSAGES[113] + '</label>' + '<input class="form-control" name="node[count]" max=100 value="1" type="text"/>' + '</div>';
                    } else {
                        // If action == edit -> print the network ID
                        html_data += '<div class="form-group col-sm-12">' + '<label class="control-label">' + MESSAGES[92] + '</label>' + '<input class="form-control" disabled name="node[id]" value="' + id + '" type="text"/>' + '</div>';
                    }
                    ...
                    function(key, value) {
                        ...
                        if (value['type'] == 'list') {
                            ...
                            html_data += '<div class="form-group ' + widthClass + '">' + '<label class=" control-label">' + value['name'] + 
                                        '</label>' + '<select class="selectpicker form-control" name="node[' + key + ']" data-size="5" data-style="selectpicker-button">';
                            $.each(value['list'],
                            function(list_key, list_value) {
                                ...
                                if (action == 'add') {
                                    ...
                                    var selected = (list_value == value_set) ? 'selected ': '';
                                }
                                iconselect = '';
                                if (key == "icon") {
                            ...
                            if (key == 'cpulimit') {
                                widthClass = ' col-sm-2 ';
                                html_data += '<div class="' + widthClass + '" style="padding-right: 0px;">' + '<label class="control-label" style="height: 34px;margin-top: 8px;margin-bottom: 0px;">' + value['name'] + '</label>' + '</div><div class="form-group col-sm-8" style="padding-left: 0px;" >' + '<input type="checkbox"  style="width: 34px;" class="form-control" value=' + values['cpulimit'] + ' name="node[' + key + ']" ' + ((values['cpulimit'] == 1) ? 'checked': '') + '/>' + '</div>';
                            }
                        } else {
                            // Option is standard
                            var widthClass = ' col-sm-12 '
                            ...
                            var postName = '';
                            if (!bothRam && template_values['options'].hasOwnProperty('cpu') && template_values['options'].hasOwnProperty('ethernet') && template_values['options'].hasOwnProperty('ram')) {
                            ...
                            html_data += '<div class="form-group' + widthClass + '">' + '<label class=" control-label"> ' + value['name'] + '<a id="link_' + key + '" onClick="javascript:document.getElementById(\'input_' + key + '\').value=\'' + template_values['options'][key]['value'] + '\';document.getElementById(\'link_' + key + '\').style.visibility=\'hidden\'" style="visibility: ' + ((value_set != template_values['options'][key]['value']) ? 'visible': 'hidden') + ';" >' + tpl + '</a>' + ram_value + '</label>' + '<input class="form-control' + ((key == 'name') ? ' autofocus': '') + '" name="node[' + key + ']" value="' + value_set + '" type="text" id="input_' + key + '" onClick="javascript:document.getElementById(\'link_' + key + '\').style.visibility=\'visible\'""/>' + '</div>';
                            if (key == 'qemu_options') {
                                html_data += '<div class="form-group' + widthClass + '">' + '<input class="form-control hidden" name="node[ro_' + key + ']" value="' + template_values['options'][key]['value'] + '" type="text" disabled/>' + '</div>';
                            }
                        }
                    });
                    html_data += '<div class="form-group col-sm-6">' + '<label class=" control-label">' + MESSAGES[93] + '</label>' + '<input class="form-control" name="node[left]" value="' + left + '" type="text"/>' + '</div>' + '<div class="form-group col-sm-6">' + '<label class=" control-label">' + MESSAGES[94] + '</label>' + '<input class="form-control" name="node[top]" value="' + top + '" type="text"/>' + '</div>';
...
function printFormNodeConfigs(values, cb) {
    var title = values['name'] + ': ' + MESSAGES[123];
    if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0) {
        var ace_themes = ['cobalt', 'github', 'crimson_editor', 'iplastic', 'draw', 'clouds_midnight', 'monokai', 'ambiance', 'chaos', 'chrome', 'clouds', 'eclipse', 'dreamweaver', 'kr_theme', 'kuroir', 'merbivore', 'idle_fingers', 'katzenmilch', 'merbivore_soft', ];
        ...
        var ace_font_size = ['12px', '13px', '14px', '16px', '18px', '20px', '24px', '28px'];
...
function printFormCustomShape(values) {
    var shapeTypes = ['Straight-sided Square', 'Rounded Square', 'Circle'],
    ...
    var html = '<form id="main-modal" class="container col-md-12 col-lg-12 custom-shape-form">' + '<div class="row">' + '<div class="col-md-8 col-md-offset-1 form-group">' + '<label class="col-md-3 control-label form-group-addon">Type</label>' + '<div class="col-md-5">' + '<select class="form-control shape-type-select">' + '</select>' + '</div>' + '</div> <br>' + '<div class="col-md-8 col-md-offset-1 form-group">' + '<label class="col-md-3 control-label form-group-addon">Name</label>' + '<div class="col-md-5">' + '<input type="text" class="form-control shape_name" placeholder="Name">' + '</div>' + '</div> <br>' + '<div class="col-md-8 col-md-offset-1 form-group">' + '<label class="col-md-3 control-label form-group-addon">Border-type</label>' + '<div class="col-md-5">' + '<select class="form-control border-type-select" >' + '</select>' + '</div>' + '</div> <br>' + '<div class="col-md-8 col-md-offset-1 form-group">' + '<label class="col-md-3 control-label form-group-addon">Border-width</label>' + '<div class="col-md-5">' + '<input type="number" min="0" value="5" class="form-control shape_border_width">' + '</div>' + '</div> <br>' + '<div class="col-md-8 col-md-offset-1 form-group">' + '<label class="col-md-3 control-label form-group-addon">Border-color</label>' + '<div class="col-md-5">' + '<input type="color" class="form-control shape_border_color">' + '</div>' + '</div> <br>' + '<div class="col-md-8 col-md-offset-1 form-group">' + '<label class="col-md-3 control-label form-group-addon">Background-color</label>' + '<div class="col-md-5">' + '<input type="color" class="form-control shape_background_color">' + '</div>' + '</div> <br>' + '<button type="submit" class="btn btn-success col-md-offset-1">' + MESSAGES[47] + '</button>' + '<button type="button" class="btn" data-dismiss="modal">' + MESSAGES[18] + '</button>' + '</div>' + '<input  type="text" class="hide left-coordinate" value="' + left + '">' + '<input  type="text" class="hide top-coordinate" value="' + top + '">' + '</form>';
...
function addTextObject(b, e) {
    ...
    c = new Date().getTime(),
    f = {};
    g = "position:absolute;left:" + resolveZoom(b, "left") + "px;top:" + resolveZoom(e, "top") + "px;";
    a = '<div id="customText' + c + '" class="customShape customText context-menu" data-path="" style="' + g + ";z-index:" + d + ';"><p>New Text</p></div>';
    f.data = a;
    f.name = "txt " + ($(".customShape").length + 1);
    f.type = "text";
    createTextObject(f).done(function(j) {
        $("#lab-viewport").prepend(a);
        var h = $("#customText" + c);
        getTextObjects().done(function(k) {
            var l = j.id;
            h.attr("id", "customText" + l);
            h.attr("data-path", l);
            if ($("#customText" + l).length > 1) {
            ...
            $("#customText" + l).dblclick()
        }).fail(function(k) {
            addMessage("DANGER", getJsonMessage(k))
        })
    }).done(function() {
        addMessage("SUCCESS", "Lab has been saved (60023).")
    }).fail(function(h) {
        addMessage("DANGER", getJsonMessage(h))
    });
    return false
}
...
function printFormNodeInterfaces(values) {
    ...
    $.when(getLabLinks()).done(function(links) {
        ...
        if (values['sort'] == 'iol') {
            ...
            var iol_interfc = {};
            $.each(values['ethernet'],
            function(interfc_id, interfc) {
                var x = interfc_id % 16;
...
function printPictureInForm(id) {
    ...
    $.when(getPictures(picture_id)).done(function(picture) {
        ...
        picture_map = picture_map.replace(/href='telnet:..{{IP}}:{{NODE([0-9]+)}}/g,
        function(a, b, c, d, e) {
            var nodehref = ''
        ...
        //var sizeClass = ""
        var body = '<div id="lab_picture">' + '<img class="' + sizeClass + '" usemap="#picture_map" ' + 'src="' + picture_url + '" ' + 'alt="' + picture['name'] + '" ' + 'title="' + picture['name'] + '" ' +
        //'width="' + picture['width'] + '" ' +
...
function printFormPicture(action, values) {
    ...
    $.when(getPictures(values['id'])).done(function(picture) {
        ...
        $.when(getNodes(null)).done(function(nodes) {
            if (action == 'add') {
                html += '<form id="form-picture-' + action + '" class="form-horizontal form-lab-' + action + '">' + '<div class="form-group">' + 
                        '<label class="col-md-3 control-label">' + MESSAGES[19] + '</label>' + '<div class="col-md-5">' + 
                        '<input class="form-control" autofocus name="picture[name]" value="' + name + '" type="text"/>' + '</div>' + 
                        '</div>' + '<div class="form-group">' + '<label class="col-md-3 control-label">' + MESSAGES[137] + '</label>' + 
                        '<div class="col-md-5">' + '<textarea class="form-control" name="picture[map]">' + map + '</textarea></div>' + '</div>' + 
                        '</div>' + '<div class="form-group">' + '<div class="col-md-5 col-md-offset-3">' + 
                ...
                html += '<form id="form-picture-' + action + '" class="form-horizontal form-lab-' + action + '" data-path=' + values['id'] + '>' + 
                        '<div class="follower-wrapper">' + '<img class="' + sizeClass + '" src="/api/labs' + $('#lab-viewport').attr('data-path') + 
                        '/pictures/' + values['id'] + '/data" alt="' + values['name'] + '" width-val="' + values['width'] + '" height-val="' + 
                        values['height'] + '"/>' + '<div id="follower">' + '<map name="picture_map">' + picture_map + '</map>' + '</div>' + 
                        '</div>' + '<div class="form-group">' + '<label class="col-md-3 control-label">' + MESSAGES[19] + '</label>' + 
                        '<div class="col-md-5">' + '<input class="form-control" autofocus name="picture[name]" value="' + name + '" type="text"/>' + 
                        '</div>' + '</div>' + '<div class="form-group">' + '<label class="col-md-3 control-label">' + MESSAGES[62] + '</label>' + 
                        '<div class="col-md-5">' + '<select class="form-control" id="map_nodeid">';
                $.each(nodes,
                function(key, value) {
                    html += '<option value="' + key + '">' + value.name + ', NODE ' + key + '</option>';
                });
                html += '<option value="CUSTOM"> CUSTOM , NODE outside lab</option>';
                html += '</select>' + '</div>' + '</div>' + '<div class="form-group">' + '<label class="col-md-3 control-label">' + MESSAGES[137] + 
                        '</label>' + '<div class="col-md-5">' + '<textarea class="form-control map hidden" name="picture[map]">' + map + '</textarea>' + 
                        '<textarea class="form-control custommap" name="picture[custommap]">' + custommap + '</textarea>' + '</div>' + '</div>' + 
                        '<div class="form-group">' + '<div class="col-md-5 col-md-offset-3">' + '<button type="submit" class="btn btn-success">' + MESSAGES[47] + 
                        '</button>' + '<button type="button" class="btn" data-dismiss="modal">' + MESSAGES[18] + '</button>' + '</div>' + '</div>' + '</form>';
            ...
            function(key, area) {
                ...
                //alert(cX + " " + cY )
                htmlsvg = '<div class="map_mark" id="' + area.coords + '" style="position:absolute;top:' + 
                        cY + 'px;left:' + cX + 'px;width:60px;height:60px;"><svg width="60" height="60"><g><ellipse cx="30" cy="30" rx="28" ry="28" ' + 
                        'stroke="#000000" stroke-width="2" fill="#ffffff"></ellipse><text x="50%" y="50%" text-anchor="middle" alignment-baseline="central" ' + 
                        'stroke="#000000" stroke-width="0px" dy=".2em" font-size="12" >' + 
                        area.href.replace(/.*{{NODE/g, "NODE ").replace(/}}/g, "").replace(/.*:.*/, "CUSTOM") + '</text></g></svg></div>'
...
function printFormUser(action, values) {
    $.when(getRoles()).done(function(roles) {
        ...
        function(e) {
            if (!$(e.target).hasClass('expiration')) $('.expiration').datepicker('hide');
        });
...
function printLabPreview(lab_filename) {
    $.when(getLabInfo(lab_filename)).done(function(lab) {
        ...
        if (ROLE != "user") html += '<button class="action-labedit-inline btn btn-flat" type="button" data-path="' + lab_filename + '">Edit</button>';
...
function createNodeListRow(template, id) {
    ...
    $.when(getTemplates(template), getNodes(id)).done(function(template_values, node_values) {
        ...
        //node boot image
        if (template == "vpcs") {
            html_data += '<td><input class="ram-value configured-nodes-input short-input readonly" data-path="' + id + '" name="node[cpu]" value="n/a" type="text" readonly /></td>';
        } else {
        ...
        html_data += '<td><input class="idle-pc-value configured-nodes-input ' + readonlyAttr + ' ' + userRight + '" data-path="' + id + '" name="node[idlepc]" value="' + value_idlepc + '" type="text" ' + readonlyAttr + ' ' + disabledAttr + ' /></td>';

        //node nvram
        readonlyAttr = (value_nvram && value_nvram != "n/a") ? "": "readonly";
        html_data += '<td><input class="nvram-value configured-nodes-input short-input ' + readonlyAttr + ' ' + userRight + '" data-path="' + id + '" name="node[nvram]" value="' + value_nvram + '" type="text" ' + readonlyAttr + ' ' + disabledAttr + ' /></td>';

        //node ram
        readonlyAttr = (value_ram && value_ram != "n/a") ? "": "readonly";
        html_data += '<td><input class="ram-value configured-nodes-input short-input ' + readonlyAttr + ' ' + userRight + '" data-path="' + id + '" name="node[ram]" value="' + value_ram + '" type="text" ' + readonlyAttr + ' ' + disabledAttr + ' /></td>';
        ...
        if (template == "iol") {
            ...
            html_data += '<td><select class="console-value configured-nods-select form-control"' + disabledAttr + ' name="node[console]" data-path="' + id + '" >'
            value_set = (node_values != null && node_values['console'] != null) ? node_values['console'] : value['value'];
            $.each(template_values['options']['console']['list'],
            function(list_key, list_value) {
                var selected = (list_key == value_set) ? 'selected ': '';
                html_data += '<option ' + selected + 'value="' + list_key + '">' + list_value + '</option>';
            });
        ...
        $.each(template_values['options']['icon']['list'],
        function(list_key, list_value) {
            var selected = (list_key == value_set) ? 'selected ': '';
        ...
        $.each(template_values['options']['config']['list'],
        function(list_key, list_value) {
            var selected = (list_key == value_set) ? 'selected ': '';
            html_data += '<option ' + selected + 'value="' + list_key + '">' + list_value + '</option>';
        });
        ...
        //node actions
        html_data += '<td><div class="action-controls">' + '<a class="action-nodestart" data-path="' + id + '" data-name="' + checkTemplateValue(template_values['options'], 'name') + '" href="javascript:void(0)" title="' + MESSAGES[66] + '"><i class="glyphicon glyphicon-play"></i></a>' + '<a class="action-nodestop" data-path="' + id + '" data-name="' + checkTemplateValue(template_values['options'], 'name') + '" href="javascript:void(0)" title="' + MESSAGES[67] + '"><i class="glyphicon glyphicon-stop"></i></a>' + '<a class="action-nodewipe" data-path="' + id + '" data-name="' + checkTemplateValue(template_values['options'], 'name') + '" href="javascript:void(0)" title="' + MESSAGES[68] + '"><i class="glyphicon glyphicon-erase"></i></a>'
        if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0) {
            html_data += '<a class="action-nodeexport" data-path="' + id + '" data-name="' + checkTemplateValue(template_values['options'], 'name') + '" href="javascript:void(0)" title="' + MESSAGES[69] + '"><i class="glyphicon glyphicon-save"></i></a> ' + '<a class="action-nodeinterfaces" data-status="' + node_values['status'] + '" data-path="' + id + '" data-name="' + checkTemplateValue(template_values['options'], 'name') + '" href="javascript:void(0)" title="' + MESSAGES[72] + '"><i class="glyphicon glyphicon-transfer"></i></a>' + '<a class="action-nodeedit control' + disabledClass + '" data-path="' + id + '" data-name="' + checkTemplateValue(template_values['options'], 'name') + '" href="javascript:void(0)" title="' + MESSAGES[71] + '"><i class="glyphicon glyphicon-edit"></i></a>' + '<a class="action-nodedelete' + disabledClass + '" data-path="' + id + '" data-name="' + checkTemplateValue(template_values['options'], 'name') + '" href="javascript:void(0)" title="' + MESSAGES[65] + '"><i class="glyphicon glyphicon-trash"></i></a>';
        }
        html_data += '</div></td></tr>';
        defer.resolve({
            "html": html_data,
...
function printListTextobjects(textobjects) {
    ...
    var text, body = '<div class="table-responsive">' + '<table class="table">' + '<thead>' + '<tr>' + '<th>' + MESSAGES[92] + '</th>' + '<th>' + MESSAGES[19] + '</th>' + '<th>' + MESSAGES[95] + '</th>' + '<th style="width:69%">' + MESSAGES[146] + '</th>' + '<th style="width:9%">' + MESSAGES[99] + '</th>' + '</tr>' + '</thead>' + '<tbody>';
    ...
    function(key, value) {
        ...
        body += '<tr class="textObject' + value['id'] + '">' + '<td>' + value['id'] + '</td>' + '<td>' + value['name'] + '</td>' + '<td>' + value['type'] + '</td>' + '<td>' + text + '</td>' + '<td>';
        if (ROLE != "user" && LOCK == 0) {
            body += '<a class="action-textobjectdelete ' + textClass + '" data-path="' + value['id'] + '" data-name="' + value['name'] + '" href="javascript:void(0)" title="' + MESSAGES[65] + '">' + '<i class="glyphicon glyphicon-trash" style="margin-left:20px;"></i>' + '</a>'
        }
        body += '</td>' + '</tr>';
    });
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data['status'] == 'success') {
                ...
                html = new EJS({
                    url: '/themes/default/ejs/layout.ejs'
                }).render({
                    "MESSAGES": MESSAGES,
                    "folder": folder,
                    "username": USERNAME,
...
function printPageLabOpen(lab) {
    ...
    $.when(printLabTopology(), getPictures()).done(function(rc, pic) {
        ...
        if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0) {
            $('#lab-sidebar ul').append('<li class="action-configsget-li"><a class="action-configsget" href="javascript:void(0)" title="' + MESSAGES[58] + '"><i class="glyphicon glyphicon-align-left"></i></a></li>');
        }
        $('#lab-sidebar ul').append('<li><a class="action-objects_box" href="javascript:void(0)" title="' + MESSAGES[267] + '"><i class="glyphicon glyphicon-folder-open"></i></a></li>');
        ...
        if ((ROLE == 'admin' || ROLE == 'editor')) {
            $('#lab-sidebar ul').append('<li><a class="action-tool_box" href="javascript:void(0)" title="' + MESSAGES[247] + '"><i class="glyphicon glyphicon-briefcase"></i></a></li>');
        }
        ...
        // $('#lab-sidebar ul').append('<li><a class="action-Connectors" href="javascript:void(0)"><i class="glyphicon glyphicon-link"></i> ' + MESSAGES[262] + '</a></li>');
        if ($.cookie("topo") == 'dark') {
	...
	if ((ROLE == 'admin'|| ROLE == 'editor')) {
        ...
        if (LOCK == 1) {
            ...
            try {
                ...
                $('.customShape').resizable().resizable("destroy").resizable({
                    grid: [3, 3],
                    autoHide: true,
                    resize: function(event, ui) {
                        textObjectResize(event, ui, {
...
function printUserManagement() {
    $.when(getUsers(null)).done(function(data) {
        ...
        var html_title = '' + '<div class="row row-eq-height"><div id="list-title-folders" class="col-md-12 col-lg-12">' + '<span title="Users">Users</span>' + '</div>' + '</div>';
...
function drawStatusInModal(data) {
    ...
    if (data['uksm'] == "enabled") {
        window.uksm = true;
        $('#ToggleUKSM').toggleCheckedState(true)
    };

    $('#ToggleKSM').toggleSwitch({
        width: "50px"
    });
    if (data['ksm'] == "enabled") {
        window.ksm = true;
    ...
    $('#ToggleCPULIMIT').toggleSwitch({
        width: "50px"
    });
    if (data['cpulimit'] == "enabled") {
...
function editTextObject(id, newData) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: JSON.stringify(newData),
        // newData is object with differences between old and new data
        success: function(data) {
            if (data['status'] == 'success') {
...
function textObjectDragStop(event, ui) {
    var id, objectData, shape_border_width;
...
function printFormConnStyle(a) {
    getTopology().done(function(k) {
        var d = ["Solid", "Dashed"],
        b = {},
        l,
        h = ["Straight", "Bezier", "Flowchart", "StateMachine"],
        e,
        c,
        j,
        n,
        m;
        tmpconn = lab_topology.getConnections().find(function(o) {
            return o.id == a
        });
        topolink = k.filter(function(o) {
            return o.source == tmpconn.source && o.source_label == tmpconn.source_label
        })[0];
        ...
        b.style = topolink.style;
        b.linkstyle = topolink.linkstyle;
        if (isIE) {
            $('input[type="color"]').hide();
        ...
        for (var f = 0; f < d.length; f++) {
            $(".edit-network-style-form .network-style-select").append($("<option></option>").val(d[f]).html(d[f]));
            if (b.style == d[f]) {
                $(".edit-network-style-form .network-style-select").val(d[f])
            }
        }
        for (var f = 0; f < h.length; f++) {
            $(".edit-network-style-form .network-linkstyle-select").append($("<option></option>").val(h[f]).html(h[f]));
            if (b.linkstyle == h[f]) {
                $(".edit-network-style-form .network-linkstyle-select").val(h[f])
            }
        }
        b.color = $("." + a.replace(/:/g, "\\:")).children().attr("stroke");
        b.label = topolink.label;
        b.labelpos = parseFloat(topolink.labelpos);
        b["pos-src"] = src_pos;
        b["pos-dst"] = dst_pos;
        console.log(b.stub);
        b.stub = (topolink.stub != undefined) ? topolink.stub: 0;
        b.beziercurviness = (topolink.beziercurviness != undefined) ? topolink.beziercurviness: 150;
        b.curviness = (topolink.curviness != undefined) ? topolink.curviness: 10;
        b.midpoint = (topolink.midpoint != undefined) ? topolink.midpoint: "0.5";
        b.round = (topolink.round != undefined) ? topolink.round: 0;
        $(".edit-network-style-form .link_color").val(b.color);
        $(".edit-network-style-form .link-label-input").val(b.label);
        $(".edit-network-style-form .link-labelpos-input").val(b.labelpos);
        $(".edit-network-style-form .network-source-pos").val(b["pos-src"]);
        $(".edit-network-style-form .network-destination-pos").val(b["pos-dst"]);
        $(".edit-network-style-form .network-style-stub").val(b.stub);
        $(".edit-network-style-form .network-style-bezier-curviness").val(b.beziercurviness);
        $(".edit-network-style-form .network-style-curviness").val(b.curviness);
        $(".edit-network-style-form .network-style-round").val(b.round);
        $(".edit-network-style-form .network-style-midpoint").val(b.midpoint);
        $(".edit-network-style-form .network-source-pos").val(b["pos-src"]);
        $(".edit-network-style-form .network-destination-pos").val(b["pos-dst"]);
        $(".edit-network-style-form .firstLinkValues-color").val(b.color);
        $(".edit-network-style-form .firstLinkValues-label").val(b.label);
        $(".edit-network-style-form .firstLinkValues-labelpos").val(b.labelpos);
        $(".edit-network-style-form .firstLinkValues-style").val(b.style);
        $(".edit-network-style-form .firstLinkValues-linkstyle").val(b.linkstyle);
        $(".edit-network-style-form .firstLinkValues-stub").val(b.stub);
        $(".edit-network-style-form .firstLinkValues-bezier-curviness").val(b.beziercurviness);
        $(".edit-network-style-form .firstLinkValues-curviness").val(b.curviness);
        $(".edit-network-style-form .firstLinkValues-round").val(b.round);
        $(".edit-network-style-form .firstLinkValues-midpoint").val(b.midpoint);
        $(".edit-network-style-form .firstLinkValues-pos-src").val(b["pos-src"]);
        $(".edit-network-style-form .firstLinkValues-pos-dst").val(b["pos-dst"])
    })
}

function printFormConnQuality(a) {
    getTopology().done(function(c) {
        var d = {};
        tmpconn = lab_topology.getConnections().find(function(e) {
            return e.id == a
        });
        topolink = c.filter(function(e) {
            return e.source == tmpconn.source && e.source_label == tmpconn.source_label
        })[0];
        var b = new EJS({
...
function printFormEditText(id) {
    ...
    var firstTextValues = {},
    transparent = false,
    colorDigits, bgColor, html = new EJS({
...
function getElementsAngle(selector) {
    ...
    tr = st.getPropertyValue("-webkit-transform") || st.getPropertyValue("-moz-transform") || st.getPropertyValue("-ms-transform") || st.getPropertyValue("-o-transform") || st.getPropertyValue("transform") || "FAIL";
...
function getLogs(file, per_page, search) {
    ...
    $.ajax({
        ...
        success: function(data) {
            var html = new EJS({
                url: '/themes/default/ejs/logs.ejs'
            }).render({
                "logs": data,
                "per_page": per_page,
                "search": search,
            ...
            var html_title = '' + '<div class="row row-eq-height"><div id="list-title-folders" class="col-md-12 col-lg-12">' + '<span title="Logs">Logs</span>' + '</div>' + '</div>';
            $('#main-title').html(html_title);
            $('#main-title').show();
...
function newConnModal(info, oe) {
    ...
    $.when(getNetworks(null), getNodes(null), getTopology()).done(function(networks, nodes, topology) {
        ...
        $.when(linksourcedata['interfaces'], linktargetdata['interfaces']).done(function(sourceif, targetif) {
            ...
            html = '<form id="addConn" class="addConn-form">' + '<input type="hidden" name="addConn[srcNodeId]" value="' + linksourcedata['id'] + '">' + 
                    ...
                    '<div class="row">' + '<div class="col-md-4">' + '<div style="text-align:center;" >' + linksourcedata['name'] + 
                    '</div>' + '<img src="' + '/images/icons/' + linksourcedata['icon'] + '" class="' + linksourcestyle + 
                    ' img-responsive" style="margin:0 auto;">' + '<div style="width:3px;height: ' + ((linksourcetype == 'net') ? '0': '10') + 
                    'px; margin: 0 auto; background-color:#444"></div>' + '<div style="margin: 0 auto; width:50%; text-align:center;" class="' + 
                    ((linksourcetype == 'net') ? 'hidden': '') + '">' + '<text class="aLabel addConnSrc text-center" >' + 
                    ((linksourcetype == 'node') ? linksourcedata['interfaces'][linksourcedata['selectedif']]['name'] : '') + 
                    '</text>' + '</div>' + '<div style="width:3px;height:160px; margin: 0 auto; background-color:#444"></div>' + 
                    '<div style="margin: 0 auto; width:50%; text-align:center;" class="' + ((linktargettype == 'net') ? 'hidden': '') + 
                    '">' + '<text class="aLabel addConnDst text-center" >' + ((linktargettype == 'node') ? linktargetdata['interfaces'][linktargetdata['selectedif']]['name'] : '') + 
                    '</text>' + '</div>' + '<div style="width:3px;height: ' + ((linktargettype == 'net') ? '0': '10') + 'px; margin: 0 auto; background-color:#444"></div>' + 
                    '<img src="/images/icons/' + linktargetdata['icon'] + '" class="' + linktargetstyle + ' img-responsive" style="margin:0 auto;">' + 
                    '<div style="text-align:center;" >' + linktargetdata['name'] + '</div>' + '</div>' + '<div class="col-md-8">' + '<div class="form-group">' + 
                    '<label>Source ID: ' + linksourcedata['id'] + '</label>' + '<p style="margin:0px;"></p>' + '<label>Source Name: ' + linksourcedata['name'] + 
                    '</label>' + '<p style="">type - ' + ((linksourcetype == 'net') ? 'Network': 'Node') + '</p>' + '</div>' + '<div class="form-group">' + 
                    '<div class="form-group ' + ((linksourcetype == 'net') ? 'hidden': '') + '">' + '<label>Choose Interface for ' + linksourcedata['name'] + 
                    '</label>' + '<select name="addConn[srcConn]" class="form-control srcConn">'
            if (linksourcetype == 'node') {
                // Eth first
                var tmp_name = [];
                var reversetab = [];
            ...
            html += '</option>'
            html += '</select>' + '<div style="width:3px;height:30px;"></div>' + '</div>' + '</div>' + '<div style="width:3px;height:30px;"></div>' + '<div class="form-group">' + '<div class="form-group ' + ((linktargettype == 'net') ? 'hidden': '') + '">' + '<label>Choose Interface for ' + linktargetdata['name'] + '</label>' + '<select name="addConn[dstConn]" class="form-control dstConn">'
            if (linktargettype == 'node') {
                // Eth first
                var tmp_name = [];
                var reversetab = [];
            ...
            html += '</option>'
            html += '</select>' + '<div style="width:3px;height:30px;"></div>' + '</div>' + '</div>' + '<div class="form-group">' + '<label>Destination ID: ' + linktargetdata['id'] + '</label>' + '<p style="margin:0px;"></p>' + '<label>Destination Name: ' + linktargetdata['name'] + '</label>' + '<p style="text-muted">type - ' + ((linktargettype == 'net') ? 'Network': 'Node') + '</p>' + '</div>' + '</div>' + '<div class="col-md-8 btn-part col-md-offset-6">' + '<div class="form-group">' + '<button type="submit" class="btn btn-success addConn-form-save">' + MESSAGES[47] + '</button>' + '<button type="button" class="btn cancelForm" data-dismiss="modal">' + MESSAGES[18] + '</button>' + '</div>' + '</div>' + '</div>' + '</form>'

            addModal(title, html, '');
        });
    });
    $('body').on('change', 'select.srcConn',
    function(e) {
        var iname = $('select.srcConn option[value="' + $('select.srcConn').val() + '"]').text();
        $('.addConnSrc').html(iname)
    });
    $('body').on('change', 'select.dstConn',
    function(e) {
...
function printFormUploadNodeConfig(path) {
    var html = '<form id="form-upload-node-config" class="form-horizontal form-upload-node-config">' + 
                '<div class="form-group">' + '<label class="col-md-3 control-label">' + MESSAGES[2] + '</label>' + 
                '<div class="col-md-5">' + '<input class="form-control" name="upload[path]" value="" disabled="" placeholder="' + MESSAGES[25] + 
                '" "type="text"/>' + '</div>' + '</div>' + '<div class="form-group">' + '<div class="col-md-7 col-md-offset-3">' + 
                '<span class="btn btn-default btn-file btn-success">' + MESSAGES[23] + 
                '<input accept="text/plain" class="form-control" name="upload[file]" value="" type="file">' + '</span>' + 
