...
$('body').on('change', 'input[name="import[file]"]',
    function(e) {
        $('input[name="import[local]"]').val($(this).val());
    });

// Choose node config upload file
$('body').on('change', 'input[name="upload[file]"]',
    function(e) {
...
//Add picture MAP
$('body').on('click', '.follower-wrapper',
    function(e) {
...
$(document).on('contextmenu', '.follower-wrapper',
    function(e) {
...
$(document).on('click', '.action-showfull-picture',
    function() {
        $('#context-menu').remove();
...
$(document).on('click', '.action-autosize',
    function() {
        $('#context-menu').remove();
...
$(document).on('click', 'a.folder, a.lab, tr.user',
    function(e) {
...
function ObjectPosUpdate(event, ui) {
    ...
    if (event.metaKey || (event.e != undefined && event.e.metaKey) || event.ctrlKey || (event.e != undefined && event.e.ctrlKey)) return logger(1, 'DEBUG: moving objects...1');
    window.moveCount += 1
    if (window.moveCount != groupMove.length) return logger(1, 'DEBUG: moving objects...2');
    var tmp_nodes = [],
        tmp_shapes = [],
        tmp_networks = [];
    $.each(groupMove,
        function(id, node) {
            ...
            if (id.search('node') != -1) {
                ...
                tmp_shapes.push({
                    id: id.replace(/customShape/, '').replace(/customText/, ''),
                    data: objectData
                })
            }
        });
...
$(document).on('click', '.menu-collapse, .menu-collapse i',
    function(e) {
...
// Open context menu block
$(document).on('click', '.menu-appear, .menu-appear i',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
...
$(document).on('contextmenu', '#lab-viewport',
    function(e) {
        // Prevent default context menu on viewport
        e.stopPropagation();
        e.preventDefault();

        $("#lab-viewport").data('contextClickXY', {
            'x': e.pageX,
            'y': e.pageY
        })
        ...
        if (window.connContext == 1) {
            ...
            if (window.connToDel.id.search("Line") == -1) {
                ...
                if ($("#" + window.connToDel.sourceId).attr("data-linkstate") == 1 || $("#" + window.connToDel.targetId).attr("data-linkstate") == 1) {
                    if (window.connToDel.id.search("iface") != -1) {
                        ...
                        var b = "endpoint_node" + window.connToDel.id.replace(/iface:node/, "").replace(/:.*/, "") + "_" + window.connToDel.id.replace(/iface:/, "").replace(/.*:/, "") + ".visible"
                    } else {
                        var d = window.connToDel.id.replace("network_id:", "");
                        var b = "networkId_" + d + ".visible"
                    }
                    if ($(".jtk-endpoint." + b).length > 0) {
                        a += '<li><a class="action-linkresume" href="javascript:void(0)"><i class="fas fa-play"></i> Resume Link</a></li>'
                    } else {
                        a += '<li><a class="action-linksuspend" href="javascript:void(0)"><i class="fas fa-pause"></i> Suspend Link</a></li>'
                    }
                    printContextMenu('Connection', a, e.pageX, e.pageY, false, "menu");
                }
                if (ROLE != "user" && LOCK != 1) {
                    deldisp = 1;
                    if ($(window.connToDel.canvas).hasClass("frame_serial")) {
                        ...
                        $.each(classList,
                            function(e, f) {
                                if (f.match(/^node\d+$/)) {
                                    current_nodes.push(f)
                                }
                            });
                        if ($("#" + current_nodes[0]).attr("data-status") > 0 || $("#" + current_nodes[1]).attr("data-status") > 0) {
                            deldisp = 0
                        }
                    }
                    if (deldisp == 1) {
                        a += '<li><a class="action-conndelete" href="javascript:void(0)"><i class="glyphicon glyphicon-trash"></i> Delete</a></li>'
                    }
                    printContextMenu('Connection', a, e.pageX, e.pageY, false, "menu");
                }
                // printContextMenu('Connection', a, e.pageX, e.pageY, false, "menu");
            ...
            if (ROLE != "user" && LOCK != 1 && window.connToDel.id.search("Line") != -1) {
                a += '<li><a class="action-linestyle" href="javascript:void(0)"><i class="glyphicon glyphicon-tint"></i> Edit Style</a></li>';
                a += '<li><a class="action-linedelete" href="javascript:void(0)"><i class="glyphicon glyphicon-trash"></i> Delete</a></li>';
                printContextMenu("Connection", a, c.pageX, c.pageY, false, "menu");
                return
            }
        }
        if (ROLE != "user" && LOCK == 0) {
            var a = '';
            a += '<li><a class="action-nodeplace" href="javascript:void(0)"><i class="glyphicon glyphicon-hdd"></i> ' + MESSAGES[81] + '</a></li>';
            a += '<li><a class="action-networkplace" href="javascript:void(0)"><i class="glyphicon glyphicon-transfer"></i> ' + MESSAGES[82] + '</a></li>';
            a += '<li><a class="action-pictureadd" href="javascript:void(0)"><i class="glyphicon glyphicon-picture"></i> ' + MESSAGES[83] + '</a></li>';
            a += '<li><a class="action-customshapeadd" href="javascript:void(0)"><i class="glyphicon glyphicon-unchecked"></i> ' + MESSAGES[145] + '</a></li>';
            a += '<li><a class="action-textadd" href="javascript:void(0)"><i class="glyphicon glyphicon-font"></i> ' + MESSAGES[146] + '</a></li>';
            a += '<li role="separator" class="divider">';
            a += '<li><a class="action-autoalign" href="javascript:void(0)"><i class="glyphicon glyphicon-th"></i> ' + MESSAGES[207] + '</a></li>';
            printContextMenu(MESSAGES[80], a, e.pageX, e.pageY, false, "menu");
        }
    });

// Manage context menu
$("body").on("click", ".action-connstyle",
    function(a) {
        logger(1, "DEBUG: action = action-connstyle");
        var b = window.connToDel.id;
        printFormConnStyle(b);
        $("#context-menu").remove()
    });

$("body").on("click", ".action-connquality",
    function(a) {
        logger(1, "DEBUG: action = action-connquality");
        var b = window.connToDel.id;
        printFormConnQuality(b);
        $("#context-menu").remove()
    });

$(document).on('contextmenu', '.context-menu',
    function(e) {
        ...
        $("#lab-viewport").data('contextMenuClickXY', {
            'x': e.pageX,
            'y': e.pageY
        })
        ...
        if ($(this).hasClass('node_frame')) {
            ...
            if (parseInt($('#node' + node_id).attr('data-status')) != 2) {
                content = '<li><a class="action-nodestart  menu-manage" data-path="' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-play"></i> ' + MESSAGES[66] + '</a>' + '</li>';
            }

            if (parseInt($('#node' + node_id).attr('data-status')) == 2) {
                content2 = '<li><a class="action-nodestop  menu-manage" data-path="' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-stop"></i> ' + MESSAGES[67] + '</a>';
            }

            var title = $(this).attr('data-name') + " (" + node_id + ")",
                body = content + content2 + '<li>' + '<a class="action-nodewipe menu-manage" data-path="' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-erase"></i> ' + MESSAGES[68] + '</a>' + '</li>' + '</li>';
            if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0) {
                body += '<li>' + '<a class="action-nodeexport" data-path="' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-save"></i> ' + MESSAGES[69] + '</a>' + '</li>';
            }
            // capture section
            body += '<li role="separator" class="divider">' + '</li>' + '<li id="menu-node-interfaces">' + '<a class="menu-appear" data-path="menu-interface" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-chevron-right"></i> ' + MESSAGES[70] + '</a>' + '<div id="capture-menu">' + '<ul></ul>' + '</div>' + '</li>';
            ...
            if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0) {
                ...
                /* +
                            ...
                            '</li>';*/
                if (!isNodeRunning) {
                    body += '<li>' + '<a class="action-nodeedit control" data-path="' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-edit"></i> ' + MESSAGES[71] + '</a>' + '</li>' + '<li>' + '<a class="action-nodedelete" data-path="' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-trash"></i> ' + MESSAGES[65] + '</a>' + '</li>';
                }
            };
            ...
            if (isFreeSelectMode) {
                ...
                body = '' + '<li>' + '<a class="action-nodestart-group context-collapsible menu-manage" href="javascript:void(0)"><i class="glyphicon glyphicon-play"></i> ' + MESSAGES[153] + '</a>' + '</li>' + '<li>' + '<a class="action-nodestop-group context-collapsible menu-manage" href="javascript:void(0)"><i class="glyphicon glyphicon-stop"></i> ' + MESSAGES[154] + '</a>' + '</li>' + '<li>' + '<a class="action-nodewipe-group context-collapsible menu-manage" href="javascript:void(0)"><i class="glyphicon glyphicon-erase"></i> ' + MESSAGES[155] + '</a>' + '</li>' + '<li>' + '<a class="action-openconsole-group context-collapsible menu-manage" href="javascript:void(0)"><i class="glyphicon glyphicon-console"></i> ' + MESSAGES[169] + '</a>' + '</li>';
                if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0) {
                    body += '' + '<li role="separator" class="divider"></li>' + '<li>' + '<a class="action-nodeexport-group context-collapsible menu-manage" href="javascript:void(0)"><i class="glyphicon glyphicon-save"></i> ' + MESSAGES[129] + '</a>' + '</li>' + '<li>' + '<a class="action-nodesbootsaved-group" href="javascript:void(0)"><i class="glyphicon glyphicon-floppy-saved"></i> ' + MESSAGES[139] + '</a>' + '</li>' + '<li>' + '<a class="action-nodesbootscratch-group" href="javascript:void(0)"><i class="glyphicon glyphicon-floppy-save"></i> ' + MESSAGES[140] + '</a>' + '</li>';
                    body += '<li role="separator" class="divider">' + '<li>' + '<a class="action-halign-group" data-path="node' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-object-align-horizontal"></i> ' + MESSAGES[204] + '</a>' + '</li>' + '<li>' + '<a class="action-valign-group" data-path="node' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-object-align-vertical"></i> ' + MESSAGES[205] + '</a>' + '</li>' + '<li>' + '<a class="action-calign-group" data-path="node' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-record"></i> ' + MESSAGES[206] + '</a>' + '</li>';
                    body += '' + '<li role="separator" class="divider"></li>' + '<li>' + '<a class="action-nodesbootdelete-group" href="javascript:void(0)"><i class="glyphicon glyphicon-trash"></i> ' + MESSAGES[159] + '</a>' + '</li>' + '<li>' + '<a class="action-nodedelete-group context-collapsible menu-manage" href="javascript:void(0)"><i class="glyphicon glyphicon-trash"></i> ' + MESSAGES[157] + '</a>' + '</li>' + '';
                }
            ...
            if (isFreeSelectMode) {
                window.contextclick = 1;
                body += '<li role="separator" class="divider">' + '<li>' + '<a class="action-halign-group" data-path=network"' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-object-align-horizontal"></i> ' + MESSAGES[204] + '</a>' + '</li>' + '<li>' + '<a class="action-valign-group" data-path="network' + node_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-object-align-vertical"></i> ' + MESSAGES[205] + '</a>' + '</li>' + '<li>' + '<a class="action-calign-group" data-path="network' + network_id + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-record"></i> ' + MESSAGES[206] + '</a>' + '</li>';
            }
            ...
            if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0) {
                ...
                var body = '<li>' + '<a class="context-collapsible  action-textobjectduplicate" href="javascript:void(0)" data-path="' + textObject_id + '">' + '<i class="glyphicon glyphicon-duplicate"></i> ' + MESSAGES[149] + '</a>' + '</li>' + '<li>' + '<a class="context-collapsible  action-textobjecttoback" href="javascript:void(0)" data-path="' + textObject_id + '">' + '<i class="glyphicon glyphicon-save"></i> ' + MESSAGES[147] + '</a>' + '</li>' + '<li>' + '<a class="context-collapsible  action-textobjecttofront" href="javascript:void(0)" data-path="' + textObject_id + '">' + '<i class="glyphicon glyphicon-open"></i> ' + MESSAGES[148] + '</a>' + '</li>' + '<li>' + '<a class="context-collapsible action-textobjectedit" href="javascript:void(0)" data-path="' + textObject_id + '">' + '<i class="glyphicon glyphicon-edit"></i> ' + MESSAGES[71] + '</a>' + '</li>' + '<li>' + '<a class="context-collapsible ' + textClass + ' action-textobjectdelete" href="javascript:void(0)" data-path="' + textObject_id + '">' + '<i class="glyphicon glyphicon-trash"></i> ' + MESSAGES[65] + '</a>' + '</li>';
                if (isFreeSelectMode) {
                    window.contextclick = 1
                    var body = '<li role="separator" class="divider">' + '<li>' + '<a class="action-halign-group" data-path="' + elId + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-object-align-horizontal"></i> ' + MESSAGES[204] + '</a>' + '</li>' + '<li>' + '<a class="action-valign-group" data-path="' + elId + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-object-align-vertical"></i> ' + MESSAGES[205] + '</a>' + '</li>' + '<li>' + '<a class="action-calign-group" data-path="' + elId + '" data-name="' + title + '" href="javascript:void(0)">' + '<i class="glyphicon glyphicon-record"></i> ' + MESSAGES[206] + '</a>' + '</li>';
                }
...
$(document).on('click', '.action-nodecapture',
    function() {
        $("#context-menu").remove();
    })
...
$(document).on('change input', 'input[name="node[count]"]',
    function(e) {
        var count = $(this).val();
...
$(document).on('mouseover', '.node_frame, .network_frame',
    function(e) {
        if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0 && ($(this).attr('data-status') == 0 || $(this).attr('data-status') == 2 || $(this).attr('data-status') == undefined) && !$('#lab-viewport').hasClass('freeSelectMode')) {
            $(this).find('.tag').removeClass("hidden");
        }
    });

$(document).on('mouseover', '.ep',
    function(e) {
        //lab_topology.setDraggable ( this , false )
    });

$(document).on('mouseleave', '.node_frame, .network_frame',
    function(e) {
...
// startup-config menu
$(document).on('click', '.action-configsget',
    function(e) {
        logger(1, 'DEBUG: action = configsget');
        $.when(getNodeConfigs(null)).done(function(configs) {
            addModalWide(MESSAGES[120], new EJS({
                url: '/themes/default/ejs/action_configsget.ejs'
            }).render({
                configs: configs
            }), '');
        }).fail(function(message) {
...
// Change opacity
$(document).on('click', '.action-changeopacity',
    function(e) {
...
// Get startup-config
$(document).on('click', '.action-configget',
    function(e) {
...
// Add a new folder
$(document).on('click', '.action-folderadd',
    function(e) {
...
$(document).on('dblclick', '.action-folderopen',
    function(e) {
...
$(document).on('click', '.action-folderrename',
    function(e) {
...
// Import labs
$(document).on('click', '.action-import',
    function(e) {
...
// Add a new lab
$(document).on('click', '.action-labadd',
    function(e) {
...
// Print lab body
$(document).on('click', '.action-labbodyget',
    function(e) {
        logger(1, 'DEBUG: action = labbodyget');
        $.when(getLabInfo($('#lab-viewport').attr('data-path')), getLabBody()).done(function(info, body) {
            addModalWide(MESSAGES[64], '<h1>' + info['name'] + '</h1><center><p><code>ID: ' + info['id'] + '</code></p><p>' + info['description'] + '</p></center>' + body, '')
...
$(document).on('click', '.action-networkedit',
    function(e) {
...
// Edit/print lab network
$(document).on('click', '.action-networkdeatach',
    function(e) {
        ...
        $.when(setNodeInterface(node_id, '', interface_id)).done(function(values) {
...
// Print lab networks
$(document).on('click', '.action-networksget',
    function(e) {
...
// Delete lab network
$(document).on('click', '.action-networkdelete',
    function(e) {
        var id = $(this).attr('data-path');
        var body = '<div class="form-group">' + '<div class="question">Are you sure to delete this network?</div>' + '</div>' + '<div class="form-group">' + '<div class="col-md-5 col-md-offset-3">' + '<button id="networkdelete" class="btn btn-success"  data-path="' + id + '" data-dismiss="modal">Yes</button>' + '<button type="button" class="btn" data-dismiss="modal">Cancel</button>' + '</div>' + '</div>'
        var title = "Warning";
        addModal(title, body, "", "make-red make-small");
    })

$(document).on('click', '.action-conndelete',
    function(e) {
...
$(document).on('contextmenu', '.map_mark',
    function(e) {
        //alert (this.id)
        e.preventDefault();
        e.stopPropagation();
...
$(document).on('click', '.action-mapdelete',
    function(e) {
...
$(document).on('click', '#networkdelete',
    function(e) {
...
// Delete lab node
$(document).on('click', '.action-nodedelete, .action-nodedelete-group',
    function(e) {
        ...
        var body = '<div class="form-group">' + '<div class="question">' + textQuestion + '</div>' + '<div class="col-md-5 col-md-offset-3">' + '<button id="deteleNode" class="btn btn-success" data-path="' + id + '" data-dismiss="modal">Yes</button>' + '<button type="button" class="btn" data-dismiss="modal">Cancel</button>' + '</div>' + '</div>'
        var title = "Warning";
        addModal(title, body, "", "make-red make-small");
        $('#context-menu').remove();
...
function recursionNodeDelete(restOfList) {
    ...
    if ($('#node' + node.path).attr('data-status') > 1) {
        addMessage('warning', node.name + ' is running! Can not be deleted!');
...
$(document).on('click', '.action-nodeinterfaces',
    function(e) {
...
$(document).on('click', '.action-nodeedit',
    function(e) {
...
// Print lab nodes
$(document).on('click', '.action-nodesget',
    function(e) {
...
// Lab close
$(document).on('click', '.action-labclose',
    function(e) {
...
// Edit a lab
$(document).on('click', '.action-labedit',
    function(e) {
...
// Edit a lab inline
$(document).on('click', '.action-labedit-inline',
    function(e) {
        logger(1, 'DEBUG: action = labedit');
...
// List all labs
$(document).on('click', '.action-lablist',
    function(e) {
...
// Open a lab
$(document).on('click', '.action-labopen',
    function(e) {
...
// Preview a lab
$(document).on('dblclick', '.action-labpreview',
    function(e) {
...
//download box
$(document).on('click', '.action-download_box',
    function(e) {
        logger(1, 'DEBUG: action = download box Menu');
        var body = '';
        body += '<li><a href="/files/EVE-NG-Win-Client-Pack-2.0.exe"><i class="fa fa-file"></i> ' + "Windows Client Package" + '</a></li>';
        body += '<li><a href="/files/windows.zip"><i class="fa fa-file"></i> ' + "Windows Manually Package" + '</a></li>';
...
$(document).on('click', '.action-fix_permissions',
    function(e) {
        $('#context-menu').remove();
        logger(1, 'DEBUG: action = Fix Permissions');
        html_loader = "<div id='progress-loader'><label style='float:left'>Fix Permissions...</label><div class='loader'></div></div>";
        $("#lab-viewport").append(html_loader);
        $.ajax({
            ...
            success: function(data) {
                ...
                if (jsonData.status == 'Success') {
                    ...
                    addMessage('success', MESSAGES[243]);
                    // $('.action-labtopologyrefresh').click();
                } else {
                    $("#progress-loader").remove();
                    addMessage('danger', MESSAGES[244]);
                }
            }
        });
    });

// Generate IOL License
$(document).on('click', '.action-generate_ioulic',
    function(e) {
        $('#context-menu').remove();
        logger(1, 'DEBUG: action = Generate IOL License');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (jsonData.status == 'Success') {
                    $("#progress-loader").remove();
                    addMessage('success', MESSAGES[245]);
                    // $('.action-labtopologyrefresh').click();
                } else {
                    $("#progress-loader").remove();
                    addMessage('danger', MESSAGES[246]);
                }
            }
        });
...
// Logout
$(document).on('click', '.action-logout',
    function(e) {
...
// Lock lab
$(document).on('click', '.action-lock-lab',
    function(e) {
...
// Unlock lab
$(document).on('click', '.action-unlock-lab',
    function(e) {
...
$(document).on('click', '.action-labobjectadd',
    function(e) {
...
// Add network
$(document).on('click', '.action-networkadd',
    function(e) {
...
// Place an object
$(document).on('click', '.action-nodeplace, .action-networkplace, .action-customshapeadd, .action-textadd',
    function(e) {
        var target = $(this),
            object,
            frame = '';
        ...
        if (object == 'node') {
            ...
            addTextObject($("#lab-viewport").data("contextClickXY").x - 30, $("#lab-viewport").data("contextClickXY").y);
        }
        $('#mouse_frame').remove();
        // }
        $('#mouse_frame').remove();
...
$(document).on('click', '.action-halign-group',
    function(e) {
        $('#context-menu').remove();
...
$(document).on('click', '.action-valign-group',
    function(e) {
        $('#context-menu').remove();
        node_id = $(this).attr('data-path');
        var target = $(this);
...
$(document).on('click', '.action-autoalign-group,.action-autoalign',
    function(e) {
        $('#context-menu').remove();
        var target = $(this);
        var zoom = $('#zoomslide').slider("value") / 100;
...
$(document).on('click', '.action-calign-group',
    function(e) {
        $('#context-menu').remove();
        var node_id = $(this).attr('data-path');
        var zoom = $('#zoomslide').slider("value") / 100;
...
$(document).on('click', '.action-openconsole-all, .action-openconsole-group',
    function(e) {
        $('#context-menu').remove();
        var target = $(this);
        var isFreeSelectMode = $("#lab-viewport").hasClass("freeSelectMode")
...
// Add picture
$(document).on('click', '.action-pictureadd',
    function(e) {
...
// Add picture form
$('body').on('submit', '#form-picture-add',
    function(e) {
...
// Edit picture
$(document).on('click', '.action-pictureedit',
    function(e) {
...
// Get picture
$(document).on('click', '.action-pictureget',
    function(e) {
...
$(document).on('mousemove', '.follower-wrapper',
    function(e) {
...
// Get pictures list
$(document).on('click', '.action-picturesget',
    function(e) {
...
$(document).on('click', '.action-picturesget-stop',
    function(e) {
        logger(1, 'DEBUG: action = picturesget');
        $.when(getPictures()).done(function(pictures) {
            if (!$.isEmptyObject(pictures)) {
...
//Detele picture
$(document).on('click', '.delete-picture',
    function(ev) {
        ev.stopPropagation(); // Prevent default behaviour
        ev.preventDefault(); // Prevent default behaviour
        var id = $(this).attr('data-path');
        console.log('this', $(this));
        var body = '<div class="form-group">' + '<div class="question">Are you sure to delete this picture?</div>' + '<div class="col-md-5 col-md-offset-3">' + '<button id="formPictureDelete" class="btn btn-success"  data-path="' + id + '" data-dismiss="modal">Yes</button>' + '<button type="button" class="btn" data-dismiss="modal">Cancel</button>' + '</div>' + '</div>'
        var title = "Warning";
        addModal(title, body, "", "make-red make-small");
        $('#formPictureDelete').on('click',
            function(e) {
...
// Clone selected labs
$(document).on('click', '.action-selectedclone',
    function(e) {
...
$(document).on('click', '.action-selecteddelete',
    function(ev) {
        var id = $(this).attr('data-path');
        var self = $(this);
        var body = '<div class="form-group">' + '<div class="question">Are you sure to delete selected nodes?</div>' + '<div class="col-md-5 col-md-offset-3">' + '<button id="selectedDelete" class="btn btn-success"  data-path="' + id + '" data-dismiss="modal">Yes</button>' + '<button type="button" class="btn" data-dismiss="modal">Cancel</button>' + '</div>' + '</div>'
        var title = "Warning";
        addModal(title, body, "", "make-red make-small");
        $('#selectedDelete').on('click',
                    ...
                    $('.selected').each(function(id, object) {
                        ...
                        if (self.hasClass('folder')) {
                            $.when(deleteFolder(path)).done(function() {
                                // Folder deleted
                                $('.folder[data-path="' + path + '"]').fadeOut(300,
                                    function() {
                                        self.remove();
                                    });
                            }).fail(function(message) {
                            ...
                            $.when(deleteLab(path)).done(function() {
                                // Lab deleted
                                $('.lab[data-path="' + path + '"]').fadeOut(300,
                                    function() {
                                        self.remove();
                                    });
                            }).fail(function(message) {
                            ...
                            $.when(deleteUser(path)).done(function() {
                                // User deleted
                                $('.user[data-path="' + path + '"]').fadeOut(300,
                                    function() {
                                        self.remove();
                                    });
                            }).fail(function(message) {
...
$(document).on('click', '.action-selectedexport',
    function(e) {
        if ($('.selected').length > 0) {
...
$(document).on('click', '.action-nodesbootdelete, .action-nodesbootdelete-group',
    function(ev) {
        $('#context-menu').remove();
        var self = $(this);
        ...
        var body = '<div class="form-group">' + '<div class="question">' + textQuestion + '</div>' + '<div class="col-md-5 col-md-offset-3">' + '<button id="nodesbootdelete" class="btn btn-success"  data-dismiss="modal">Yes</button>' + '<button type="button" class="btn" data-dismiss="modal">Cancel</button>' + '</div>' + '</div>'
        var title = "Warning";
        addModal(title, body, "", "make-red make-small");
        $('#nodesbootdelete').on('click',
            function(e) {
                var isFreeSelectMode = $("#lab-viewport").hasClass("freeSelectMode");
                if (isFreeSelectMode) {
                    var nodeLenght = window.freeSelectedNodes.length;
                    var lab_filename = $('#lab-viewport').attr('data-path');
                    $.each(window.freeSelectedNodes,
                        function(i, node) {
                            var form_data = {};
...
$(document).on('click', '.action-nodesbootscratch, .action-nodesbootscratch-group',
    function(e) {
        $('#context-menu').remove();
...
$(document).on('click', '.action-nodesbootsaved, .action-nodesbootsaved-group',
    function(e) {
        $('#context-menu').remove();
...
// Export a config
$(document).on('click', '.action-nodeexport, .action-nodesexport, .action-nodeexport-group',
    function(e) {
        $('#context-menu').remove();

        var node_id, isFreeSelectMode = $("#lab-viewport").hasClass("freeSelectMode"),
            exportAll = false,
            nodesLength;
        ...
        $.when(getNodes(null)).done(function(nodes) {
            if (isFreeSelectMode) {
                ...
                $.when(recursive_cfg_export(window.freeSelectedNodes, nodesLenght)).done(function() {}).fail(function(message) {
                ...
                $.when(recursive_cfg_export(nodes, nodesLenght)).done(function() {}).fail(function(message) {
...
// Start a node
$(document).on('click', '.action-nodestart, .action-nodesstart, .action-nodestart-group',
    function(e) {
        $('#context-menu').remove();
        var node_id, startAll, isFreeSelectMode = $("#lab-viewport").hasClass("freeSelectMode"),
            nodeLenght;
        ...
        $.when(getNodes(null)).done(function(nodes) {
            if (isFreeSelectMode) {
                ...
                $.when(recursive_start(window.freeSelectedNodes, nodeLenght)).done(function() {}).fail(function(message) {
                ...
                $.when(start(node_id)).done(function() {
                    ...
                    if ($('input[data-path=' + node_id + '][name="node[type]"]') && $('input[data-path=' + node_id + '][name="node[type]"]').parent()) {
                ...
                $.when(recursive_start(nodes, nodesLenght)).done(function() {}).fail(function(message) {
                    addMessage('danger', 'Start all: Error');
                });
                /*
...
// Stop a node
$(document).on('click', '.action-nodestop, .action-nodesstop, .action-nodestop-group',
    function(e) {
        $('#context-menu').remove();

        var node_id, nodeLenght, isFreeSelectMode = $("#lab-viewport").hasClass("freeSelectMode"),
            stopAll;
...
// Wipe a node
$(document).on('click', '.action-nodewipe, .action-nodeswipe, .action-nodewipe-group',
    function(e) {
        $('#context-menu').remove();
        var self = $(this);
        var textQuestion = "";
        ...
        var body = '<div class="form-group">' + '<div class="question">' + textQuestion + '</div>' + '<div class="col-md-5 col-md-offset-3">' + '<button id="node_wipe" class="btn btn-success"  data-dismiss="modal">Yes</button>' + '<button type="button" class="btn" data-dismiss="modal">Cancel</button>' + '</div>' + '</div>'
        var title = "Warning";
        addModal(title, body, "", "make-red make-small");
        $('#node_wipe').on('click',
            function(ev) {
...
// Stop all nodes
$(document).on('click', '.action-stopall',
    function(e) {
...
$(document).on('click', '.action-sysstatus',
    function(e) {
        bodyAddClass('status');
        ...
        $.when(getSystemStats()).done(function(data) {
            // Main: title
            var html_title = '' + '<div class="row row-eq-height"><div id="list-title-folders" class="col-md-12 col-lg-12">' + '<span title="' + MESSAGES[13] + '">' + MESSAGES[13] + '</span>' + '</div>' + '</div>';

            // Main
            var html = '' + '<div id="systemStats" class="container col-md-12 col-lg-12">' + '<div class="fill-height row row-eq-height">' + '<div id="stats-text" class="col-md-3 col-lg-3">' + '<ul></ul>' + '</div>' + '<div id="stats-graph" class="col-md-9 col-lg-9">' + '<ul></ul>' + '</div>' + '</div>' + '</div>';
...
// Add a user
$(document).on('click', '.action-useradd',
    function(e) {
...
// Edit a user
$(document).on('dblclick', '.action-useredit',
    function(e) {
...
$(document).on('click', '.action-update',
    function(e) {
...
// Load user management page
$(document).on('click', '.action-usermgmt',
    function(e) {
        bodyAddClass('users');
...
// Show status
$(document).on('click', '.action-status',
    function(e) {
        logger(1, 'DEBUG: action = show status');
        $.when(getSystemStats()).done(function(data) {

            // Body
            var html = '<div id="statusModal" class="container col-md-12 col-lg-12">' + '<div class="fill-height row row-eq-height">' + '<div id="stats-text" class="col-md-3 col-lg-3">' + '<ul></ul>' + '</div>' + '<div id="stats-graph" class="col-md-9 col-lg-9">' + '<ul></ul>' + '</div>' + '</div>' + '</div>';
...
// Submit folder form
$(document).on('submit', '#form-folder-add, #form-folder-rename',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
...
// Submit lab form
$(document).on('submit', '#form-lab-add, #form-lab-edit',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
        var lab_filename = $('#lab-viewport').attr('data-path');
...
// Submit network form
$(document).on('submit', '#form-network-add, #form-network-edit',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
        var lab_filename = $('#lab-viewport').attr('data-path');
...
$(document).on('submit', '#form-node-connect',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
        var lab_filename = $('#lab-viewport').attr('data-path');
...
$(document).on('submit', '#form-node-add, #form-node-edit',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
        var self = $(this);
        var lab_filename = $('#lab-viewport').attr('data-path');
...
// Submit node form
$(document).on('submit', '#oldform-node-add, #oldform-node-edit',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
        var self = $(this);
        var lab_filename = $('#lab-viewport').attr('data-path');
...
$(document).on('focusout', '.configured-nodes-input',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
        var id = $(this).attr('data-path');
...
$(document).on('focusout', '.configured-nods-select',
    function(e) {
        console.log("here");
        var id = $(this).attr('data-path');
        $('input[data-path=' + id + '][name="node[type]"]').parent().removeClass('node-editing')
...
$(document).on('change', '.configured-nods-select',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
        var id = $(this).attr('data-path');
        setNodeData(id);
...
$(document).on('focus', '.configured-nods-select, .configured-nodes-input',
    function(e) {
        var id = $(this).attr('data-path');
...
// Submit config form
$(document).on('submit', '#form-node-config',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
...
// Submit user form
$(document).on('submit', '#form-user-add, #form-user-edit',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
...
// Edit picture form
$('body').on('submit', '#form-picture-edit',
    function(e) {
        e.preventDefault(); // Prevent default behaviour
        var picture_id = $(this).attr('data-path');
        var missed_id = []
        ...
        $.when(getNodes(null)).then(function(nodes) {
            ...
            if (missed_id.length > 0) {
                ...
                if (missed_id.length > 1) {
                    body += '<div class="question">Nodes IDs does not exist: ' + missed_id.join(', ') + '</div>' + '<div class="question">Please change it to the correct value</div>'
                } else {
                    body += '<div class="question">Node ID does not exist: ' + missed_id[0] + '</div>' + '<div class="question">Please change it to the correct value</div>'
                }
                body += '<div class="col-md-5 col-md-offset-3">' + '<button class="btn" data-dismiss="modal">Cotinue edit picture</button>' +
...
// Edit picture form
$('body').on('submit', '#form-picture-delete',
    function(ev) {
...
$('body').on('mouseover', '.ui-resizable-handle',
    function(e) {
        lab_topology.setDraggable($('.customShape'), false)
    });

$('body').on('mouseleave', '.ui-resizable-handle',
    function(e) {
        if (LOCK == 0) lab_topology.setDraggable($('.customShape'), true)
    });
// Add Custom Shape
$('body').on('submit', '.custom-shape-form',
    function(e) {
        var shape_options = {},
            shape_html, dashed = '',
            ...
            dash_line_length = '10',
            z_index = 999,
            radius, coordinates, current_lab, customShape_id = '',
            generateName = false;
        ...
        if (shape_options['shape_type'] == 'Straight-sided Square' || shape_options['shape_type'] == 'Rounded Square') {
            shape_html = '<div id="customShape' + shape_options['id'] + '" class="customShape context-menu" data-path="' +
                customShape_id + '" ' + 'style="display:inline;z-index:' + z_index + ';' + coordinates + '" ' +
                'width="' + shape_options['shape_width/height'] + 'px" height="' + shape_options['shape_width/height'] +
                'px" >' + '<svg width="' + shape_options['shape_width/height'] + '" height="' +
                shape_options['shape_width/height'] + '">' + '<rect ' + 'x="' + shape_options['shape_border_width'] +
                '" y="' + shape_options['shape_border_width'] + '" '
            if (shape_options['shape_type'] == "Rounded Square") {
            ...
            shape_html += 'width="' + (shape_options["shape_width/height"] - shape_options['shape_border_width'] * 2) +
                ...
                '" fill ="' + shape_options['shape_background_color'] + '" stroke-width ="' +
                shape_options['shape_border_width'] + '" stroke ="' + shape_options['shape_border_color'] +
                '" ' + dashed + "/>Sorry, your browser does not support inline SVG.</svg></div>";
        } else if (shape_options['shape_type'] == 'Circle') {
            radius = shape_options['shape_width/height'] / 2 - shape_options['shape_border_width'] / 2;

            shape_html = '<div id="customShape' + shape_options['id'] + '" class="customShape context-menu" data-path="' + customShape_id + '" ' + 'style="display:inline;z-index:' + z_index + ';' + coordinates + '"' + 'width="' + shape_options['shape_width/height'] + 'px" height="' + shape_options['shape_width/height'] + 'px" >' + '<svg width="' + shape_options['shape_width/height'] + '" height="' + shape_options['shape_width/height'] + '">' + '<ellipse cx="' + (radius + shape_options['shape_border_width'] / 2) + '" ' + 'cy="' + (radius + shape_options['shape_border_width'] / 2) + '" ' + 'rx="' + radius + '" ' + 'ry="' + radius + '" ' + 'stroke ="' + shape_options['shape_border_color'] + '" ' + 'stroke-width="' + shape_options['shape_border_width'] / 2 + '" ' + dashed + 'fill ="' + shape_options['shape_background_color'] + '" ' + '/>' + 'Sorry, your browser does not support inline SVG.' + '</svg>' + '</div>';
        }
...
// Add Text
$('body').on('submit', '.add-text-form',
    function(e) {
        var text_options = {},
            text_html, coordinates, z_index = 1001,
            text_style = '',
            customShape_id = '',
            form_data = {};
        ...
        text_html = '<div id="customText' + text_options['id'] + '" class="customShape customText context-menu" data-path="' + customShape_id + '" ' + 'style="display:inline;' + coordinates + ' cursor:move; ;z-index:' + z_index + ';" >' + '<p align="' + text_options['alignment'] + '" style="' + 'vertical-align:' + text_options['vertical-alignment'] + ';' + 'color:' + text_options['color'] + ';' + 'background-color:' + text_options['background-color'] + ';' + 'font-size:' + text_options['text-size'] + 'px;' + text_style + '">' + text_options['text'] + '</p>' + '</div>';
...
$('body').on('click', '.action-textobjectduplicate',
    function(e) {
...
$('body').on('click', '.action-textobjecttoback',
    function(e) {
...
$('body').on('click', '.action-textobjecttofront',
    function(e) {
...
$('body').on('click', '.action-textobjectedit',
    function(e) {
...
$('body').on('click', '.action-textobjectdelete',
    function(ev) {
        $('#context-menu').remove();
        var id = $(this).attr('data-path');
        var self = $(this);
        var textQuestion = $(this).hasClass('customText') ? 'Are you sure to delete this text?' : 'Are you sure to delete this shape?'
        var body = '<div class="form-group">' + '<div class="question">' + textQuestion + '</div>' + '<div class="col-md-5 col-md-offset-3">' + '<button id="textobjectdelete" class="btn btn-success"  data-path="' + id + '" data-dismiss="modal">Yes</button>' + '<button type="button" class="btn" data-dismiss="modal">Cancel</button>' + '</div>' + '</div>'
        var title = "Warning";
        addModal(title, body, "", "make-red make-small");
        $('#textobjectdelete').on('click',
            function(e) {
...
$('body').on('contextmenu', '.edit-custom-shape-form, .edit-custom-text-form, #context-menu',
    function(e) {
        e.preventDefault();
        e.stopPropagation();
    });
...
$('body').on('click', '.edit-custom-text-form .btn-align-left',
    function(e) {
...
$('body').on('click', '.edit-custom-text-form .btn-align-center',
    function(e) {
...
$('body').on('click', '.edit-custom-text-form .btn-align-right',
    function(e) {
        logger(1, 'DEBUG: action = action-set/delete left alignment');
        var id = $(this).attr('data-path');
...
$('body').on('click', '.edit-custom-text-form .btn-text-italic',
    function(e) {
...
$('body').on('click', '.edit-custom-text-form .btn-text-bold',
    function(e) {
...
$('body').on('change', '.edit-custom-text-form .text-z_index-input',
    function(e) {
...
$('body').on('change', '.edit-custom-text-form .text_background_color',
    function(e) {
...
$('body').on('click', '.edit-custom-text-form .text_background_transparent',
    function(e) {
        logger(1, 'DEBUG: action = action-change text background color');
        var id = $(this).attr('data-path');
...
$('body').on('change', '.edit-custom-text-form .text_color',
    function(e) {
...
$('body').on('change', '.edit-custom-text-form .text-rotation-input',
    function(e) {
...
$('body').on('click', '.edit-custom-text-form .cancelForm',
    function(e) {
...
$('body').on('click', '.edit-custom-text-form-save',
    function(e) {
...
$(document).on("dblclick", ".customText",
    function(a) {
        ...
        $("#lab-viewport").selectable().selectable("disable");
        var c = $(this).attr("data-path"),
            b = $("#customText" + c + " p");
        try {
            lab_topology.setDraggable("customText" + c, false)
        } catch (a) {
            console.warn(a)
        }
        $("#customText" + c).attr("contenteditable", "true").addClass("editable").focus();
        $("#customText" + c).css("cursor", "auto");
        $("#lab-viewport").selectable().selectable("destroy");
        window.ck = "customText" + c;

        //    CKEDITOR.config.language = 'en';
        CKEDITOR.inline(window.ck)
    });

$(document).on('paste', '[contenteditable="true"]',
    function(e) {
        e.preventDefault();
        var text = null;
...
//$(document).on('focusout', '.editable',
//function(e) {
//    $("#lab-viewport").selectable("enable");
//    var new_data, id = $(this).parent().attr('data-path'),
//    $selected_shape = $("#customText" + id),
...
$('body').on('click', '.edit-custom-shape-form .cancelForm',
    function(e) {
...
$('body').on('change', '.edit-custom-shape-form .shape-z_index-input',
    function(e) {
...
$('body').on('change', '.edit-custom-shape-form .shape_border_width',
    function(e) {
...
$('body').on('change', '.edit-custom-shape-form .border-type-select',
    function(e) {
...
$('body').on('change', '.edit-custom-shape-form .shape_background_color',
    function(e) {
...
$('body').on('click', '.edit-custom-shape-form .shape_background_transparent',
    function(e) {
        logger(1, 'DEBUG: action = action-change shape background color');
...
$('body').on('change', '.edit-custom-shape-form .shape_border_color',
    function(e) {
...
$('body').on('change', '.edit-custom-shape-form .shape-rotation-input',
    function(e) {
        logger(1, 'DEBUG: action = action-rotate shape');
        var id = $(this).attr('data-path'),
            angle = parseInt(this.value);
...
$('body').on('click', '.edit-custom-shape-form-save',
    function(e) {
...
$(document).on('click', '.action-textobjectsget',
    function(e) {
...
$(document).on("click", ".action-freeselect",
    function(event) {
        var self = this,
...
$(document).on("click", "#lab-viewport.freeSelectMode .onode_frame",
    function(event) {
...
$(document).on("click", ".user-settings",
    function() {
...
$(document).on('click', 'a.interfaces.serial',
    function(e) {
        e.preventDefault();
    })

$(document).on("click", "#lab-viewport",
    function(c) {
        var context = 0;
        try {
            if (c.target.className.search("action-") != -1 || $("#context-menu").length > 0) context = 1;
        } catch (a) {}
        if (!c.metaKey && !c.ctrlKey && $(this).hasClass("freeSelectMode") && window.dragstop != 1 && context == 0) {
            $(".free-selected").removeClass("free-selected");
            $(".ui-selected").removeClass("ui-selected");
            $(".ui-selecting").removeClass("ui-selecting");
            $("#lab-viewport").removeClass("freeSelectMode");
        ...
        if ($(c.target.offsetParent).is(".editable") == false) {
            if (window.ck != null) {
                CKEDITOR.instances[window.ck].destroy();
                tid = window.ck.replace("customText", "");
                new_data = document.getElementById("customText" + tid).outerHTML;
                editTextObject(tid, {
                    data: new_data
                })
            }
...
$(document).on('mousedown', '.network_frame, .node_frame, .customShape',
    function(e) {
        if (e.which == 1) {
...
// Reset Lab Zoom
$(document).on('click', '.sidemenu-zoom',
    function(e) {
        var zoom = 1;
...
$(document).on('click', '.node.node_frame a',
    function(e) {
        ...
        if (!status) {
            ...
            $.when(getNodes(node_id)).then(function(node) {

                var network = '<li><a class="action-nodestart menu-manage" data-path="' + node_id + '" data-name="' + node.name + '" href="#"><i class="glyphicon glyphicon-play"></i> Start</a></li>';
                if ((ROLE == 'admin' || ROLE == 'editor') && LOCK == 0) {
                    network += '<li><a style="display: block;" class="action-nodeedit " data-path="' + node_id + '" data-name="' + node.name + '" href="#"><i class="glyphicon glyphicon-edit"></i> Edit</a></li>';
                }
...
// CPULIMIT Toggle
$(document).on('change', '#ToggleCPULIMIT',
    function(e) {
...
// Import labs
$(document).on('click', '.action-upload-node-config',
    function(e) {
        logger(1, 'DEBUG: action = import');
        printFormUploadNodeConfig($('#list-folders').attr('data-path'));
    });

$(document).on('submit', '#form-upload-node-config',
    function(e) {
        e.preventDefault();
...
$(document).on('click', '.configured-nodes-checkbox',
    function(e) {
        var id = $(this).attr('data-path');
        setNodeData(id);
    });

$(document).on('click', '.action-nightmode',
    function(e) {
...
$(document).on('click', '.action-lightmode',
    function(e) {
