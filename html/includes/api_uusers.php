...
function apiGetUUser($db, $user) {
	...
	$query = 'SELECT users.username AS username, email, users.expiration AS expiration, name, password, session, role, ip, pods.id AS pod, pods.expiration AS pexpiration FROM users LEFT JOIN pods ON users.username = pods.username WHERE users.username = :username;';
