...
function addTap($s, $u)
{
    ...
    $cmd = 'sysctl -w net.ipv6.conf.' . $s . '.disable_ipv6=1';
    error_log(date('M d H:i:s ') . 'INFO: ' . $cmd);
    exec($cmd, $o, $rc);
    if ($rc != 0)
    {
...
function SetQuality($p)
{
    ...
    $i = isset($p['destination_interface']) ? Array(
        $p['source_interface'],
        $p['destination_interface']
    ) : Array(
        $p['source_interface']
    );
    $delay = Array(
    ...
    $bandwidth = Array(
        $p['source_bandwidth'],
        $p['destination_bandwidth']
    );

    foreach ($i as $key => $value)
    {
        $cmd = 'sudo tc qdisc replace dev ' . $value . ' root netem delay ' . $delay[$key] . 'ms ' . $jitter[$key] . 'ms' . ' loss ' . $loss[$key] . ' rate ' . $bandwidth[$key] . 'Kbit 2>&1';
        exec($cmd, $o, $rc);
...
function checkUsername($i)
{
    ...
    if ($rc != 0)
    {
        ...
        $cmd = '/usr/sbin/useradd -c "Unified Networking Lab TID=' . $i . '" -d ' . $path . ' -g unl -M -s /usr/sbin/nologin -u ' . $uid . ' unl' . $i . ' 2>&1';
        exec($cmd, $o, $rc);
        if ($rc != 0)
        {
...
/**
 * Function to disconnect an interface (TAP) to a network (Bridge/OVS)
 *
 * @param   string  $n                  Network name
 * @param   string  $p                  Interface name
 * @return  int                         0 means ok
...
function disconnectInterface($n, $p)
{
    if (isBridge($n))
    {
        ...
        {
            // Failed to delete interface to Bridge
    ...
    else if (isOvs($n))
    {
        ...
        {
            // Failed to delete interface to OVS
...
function prepareNode($n, $id, $t, $nets)
{
    ...
    foreach ($n->getEthernets() as $interface_id => $interface)
    {
        ...
        if (isset($nets[$interface->getNetworkId() ]) && $nets[$interface->getNetworkId() ]
            ->isCloud())
        {
            // Network is a Cloud
            $net_name = $nets[$interface->getNetworkId() ]
                ->getNType();
        }
        else
        {
    ...
    if (!is_file($n->getRunningPath() . '/.prepared') && !is_file($n->getRunningPath() . '/.lock'))
    {
        ...
        switch ($n->getNType())
        {
            ...
            case 'iol':
                ...
                if (file_exists('/opt/unetlab/addons/iol/bin/' . $n->getImage()))
                {
                    symlink('/opt/unetlab/addons/iol/bin/' . $n->getImage() , $n->getRunningPath() . '/' . $n->getImage());
                }
        ...
        if ($n->getConfig() == '1' && $n->getConfigData() != '')
        {
            ...
            else
            {
                switch ($n->getTemplate())
                {
                    ...
                    break;
                    case 'paloalto':
                        exec('mkdir ' . $n->getRunningPath() . '/cdrom');
                        exec('mkdir ' . $n->getRunningPath() . '/cdrom/config');
                        exec('mkdir ' . $n->getRunningPath() . '/cdrom/license');
                        exec('mkdir ' . $n->getRunningPath() . '/cdrom/software');
                        exec('mkdir ' . $n->getRunningPath() . '/cdrom/content');
                        copy($n->getRunningPath() . '/startup-config', $n->getRunningPath() . '/cdrom/config/bootstrap.xml');
                        preg_match_all('/(\s*<system>.*<\/system>)/ms', file_get_contents($n->getRunningPath() . '/startup-config') , $xmlinit);
                        ...
                        file_put_contents($n->getRunningPath() . '/cdrom/config/init-cfg.txt', $initcfg);
                        $isocmd = 'mkisofs -o ' . $n->getRunningPath() . '/config.iso -l --iso-level 4 ' . $n->getRunningPath() . '/cdrom';
                        exec($isocmd, $o, $rc);
                        //exec('rm -rf '.$n -> getRunningPath().'/cdrom/');
...
function start($n, $id, $t, $nets, $scripttimeout)
{
    ...
    if (($n->getTemplate() == 'paloalto') && is_file($n->getRunningPath() . '/config.iso') && !is_file($n->getRunningPath() . '/.configured') && $n->getConfig() != 0)
    {
    ...
    exec($cmd, $o, $rcp);
    if ($rcp == 0 && $n->getNType() == 'qemu' && is_file($n->getRunningPath() . '/startup-config') && !is_file($n->getRunningPath() . '/.configured') && $n->getConfig() != 0)
    {
    ...
    if ($rcp == 0 && $n->getNType() == 'qemu' && $n->getCpuLimit() === 1)
    {
        sleep(1);
...
function stop($n)
{
    if ($n->getStatus() != 0)
    {
        ...
        else
        {
            ...
            $cmd = 'fuser -k -TERM ' . $n->getRunningPath() . ' > /dev/null 2>&1';
        }
