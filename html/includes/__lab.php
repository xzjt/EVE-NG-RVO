...
class Lab {
	...
	public function __construct($f, $tenant) {
		...
		if (!is_file($f)) {
			...
			foreach ($xml -> xpath('//lab/topology/nodes/node') as $node_id => $node) {
				...
				if (isset($node -> attributes() -> qemu_options)) $n['qemu_options'] = (string) $node -> attributes() -> qemu_options;
				if (isset($node -> attributes() -> qemu_version)) $n['qemu_version'] = (string) $node -> attributes() -> qemu_version;
				if (isset($node -> attributes() -> qemu_arch)) $n['qemu_arch'] = (string) $node -> attributes() -> qemu_arch;
				if (isset($node -> attributes() -> qemu_nic)) $n['qemu_nic'] = (string) $node -> attributes() -> qemu_nic;
				...
				if ( $n['template']  == "bigip" ||  $n['template']  == "firepower6" ||  $n['template']  == "firepower" ||  $n['template']  == "linux" ) {
				...
				try {
					...
					error_log(date('M d H:i:s ').'WARNING: is template '.$n['template'].' valid ? '.$GLOBALS['messages'][20010]);
					error_log(date('M d H:i:s ').(string) $e);
					//continue;
					throw new Exception('invalid node template '.$n['template']);
				}
				...
				foreach ($node -> interface as $interface) {
					...
					if (isset($interface -> attributes() -> beziercurviness)) $i['beziercurviness'] = (string) $interface -> attributes() -> beziercurviness;
					if (isset($interface -> attributes() -> round)) $i['round'] = (string) $interface -> attributes() -> round;
					if (isset($interface -> attributes() -> midpoint)) $i['midpoint'] = (string) $interface -> attributes() -> midpoint;
			...
			foreach ($this -> nodes as $node_id => $node) {
				foreach ($node -> getInterfaces() as $interface_id => $interface) {
					if ($interface -> getNetworkId() == $i) {
						if ($this -> networks[$i] -> isCloud()) {
							// if it is a Cloud
							$net_name = $this -> networks[$i] -> getNType();
						} else {
							// else is a bridge
							$net_name = 'vnet'.$this -> getTenant().'_'.$interface -> getNetworkId();
						}
						$node_interface = 'vunl'.$this -> getTenant().'_'.$node_id.'_'.$interface_id;
						$rc = disconnectInterface($net_name, $node_interface);

						// if it is a bridge, delete it
						if (preg_match('/vnet[0-9]+/', $net_name)) {
	...
	public function deleteNode($i) {
		if (isset($this -> nodes[$i])) {
			...
			// delete connecting interface
			foreach ($node -> getInterfaces() as $interface_id => $interface){
				if (isset($this -> networks[$interface -> getNetworkId()]) && $this -> networks[$interface -> getNetworkId()] -> isCloud()){
					$net_name = $this -> networks[$interface -> getNetworkId()] -> getNType();
				}else{
					$net_name = 'vnet'.$this -> getTenant().'_'.$interface -> getNetworkId();
				}
				$interface_name = 'vunl'.$this -> getTenant().'_'.$i.'_'.$interface_id;
				$rc = disconnectInterface($net_name, $interface_name);
				
				// ensure the return isn't 0 to aviod issue occurs
				if ($interface -> getNetworkId() >= 1) {
					// verify if network has node is connecting
					if ($this -> networks[$interface -> getNetworkId()] -> getCount() == 2 && preg_match('/vnet[0-9]+/', $net_name)) {
						$rc = delBridge($net_name);
					}
				}				
			}
	...
	public function editNetwork($p) {
		if (!isset($this -> networks[$p['id']])) {
			...
			foreach ($this -> nodes as $node_id => $node) {
				foreach ($node -> getInterfaces() as $interface_id => $interface) {
					...
					if ($NET_ID == $p['id'] && $this -> networks[$NET_ID] -> isCloud()) {
						$net_name = $this -> networks[$NET_ID] -> getNType();
						$node_interface = 'vunl'.$this -> getTenant().'_'.$node_id.'_'.$interface_id;
						$rc = disconnectInterface($net_name, $node_interface);
					} else if ($NET_ID == $p['id']) {
						// define components
						$net_name = 'vnet'.$this -> getTenant().'_'.$p['id'];
						$node_interface = 'vunl'.$this -> getTenant().'_'.$node_id.'_'.$interface_id;
						// perform disconnection
						$rc = disconnectInterface($net_name, $node_interface);
						$rc1 = delBridge($net_name);
					}
				}
			}
		}
		...
		if (!isset($this -> networks[$p['id']])) {
			...
			if ( isset ( $p['save'] ) && $p['save'] === 0) {
				...
				// Method for node connect to net
				foreach ($this -> nodes as $node_id => $node) {
					foreach ($node -> getInterfaces() as $interface_id => $interface) {
						$NET_ID = $interface -> getNetworkId();
						if ($NET_ID == $p['id'] && $this -> networks[$NET_ID] -> isCloud()) {
							$net_name = $this -> networks[$NET_ID] -> getNType();
							$node_interface = 'vunl'.$this -> getTenant().'_'.$node_id.'_'.$interface_id;
							$rc = connectInterface($net_name, $node_interface);
						} else if ($NET_ID == $p['id']) {
							// define components
							$net_name = 'vnet'.$this -> getTenant().'_'.$p['id'];
							$node_interface = 'vunl'.$this -> getTenant().'_'.$node_id.'_'.$interface_id;
							// perform connection
							$rc = addBridge(Array('name' => $net_name));
							$rc1 = connectInterface($net_name, $node_interface);
						}
					}
				}
				return $this -> save();
			...
			if ($interface_link !== '') {
				...
				if (strpos($interface_link, ':') === False) {
					...
					if (isset($this -> getNetworks()[$interface_link])) {
						...
						$link_status = $this -> nodes[$n] -> linkInterface($i);	
				...
				$node = $this -> nodes[$n];
				$interface_link = $node -> getInterfaces()[$interface_id] -> getNetworkID();
				...
				foreach ($this -> getNodes() as $node_id => $node) {
					...
					switch ($node -> getNType()) {
						...
						case 'qemu':
							...
							if ( $node -> getQemu_options() != '' ) $d -> addAttribute('qemu_options', $node -> getQemu_options());
							if ( $node -> getQemu_version() != '' ) $d -> addAttribute('qemu_version', $node -> getQemu_version());
							if ( $node -> getQemu_arch() != '' ) $d -> addAttribute('qemu_arch', $node -> getQemu_arch());
							if ( $node -> getQemu_nic() != '' ) $d -> addAttribute('qemu_nic', $node -> getQemu_nic());
							break;
					}
