...
function html5AddSession($db, $name, $type, $port, $userid) {
	...
	$query = "insert into guacamole_connection_permission ( user_id, connection_id, permission ) values ( ".($userid+1000)." , ".$port.", 'UPDATE' ) ON DUPLICATE KEY UPDATE connection_id='$port',permission='UPDATE';";
	$statement = $db -> prepare($query);
	$statement -> execute();
