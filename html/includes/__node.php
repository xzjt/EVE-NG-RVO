...
class Node
{
    ...
    public function getCommand()
    {
        ...
        if ($this->type == 'dynamips')
        {
            ...
            $p = yaml_parse_file(BASE_DIR . '/html/' . TPL_DIR . '/' . $this->template . '.yml');
        ...
        if ($this->type == 'qemu')
        {
            ...
            */
            ...
            // Adding custom flags
            $qoptions = ($this->getQemu_options() != "") ? addslashes($this->getQemu_options()) : (isset($p['qemu_options']) ? addslashes($p['qemu_options']) : "");
    ...
    {
        $p = yaml_parse_file(BASE_DIR . '/html/' . TPL_DIR . '/' . $this->template . '.yml');
        if ($this->qemu_options == '')
        {
            return isset($p['qemu_options']) ? $p['qemu_options'] : '';
        }
        else
        {
    ...
    public function getQemu_arch()
    {
        ...
        if ($this->qemu_arch == '')
        {
            return isset($p['qemu_arch']) ? $p['qemu_arch'] : '';
        }
        else
        {
    ...
    {
        $p = yaml_parse_file(BASE_DIR . '/html/' . TPL_DIR . '/' . $this->template . '.yml');
        if ($this->qemu_version == '')
        {
            return isset($p['qemu_version']) ? $p['qemu_version'] : '2.4.0';
        }
        else
        {
    ...
    public function getQemu_nic()
    {
        ...
        if ($this->qemu_nic == '')
        {
            return isset($p['qemu_nic']) ? $p['qemu_nic'] : '';
        }
        else
        {
    ...
    public function setEthernets()
    {
        ...
        switch ($this->type)
        {
                ...
                switch ($this->getTemplate())
                {
                    ...
                    case 'c1710':
                        ...
                        else
                        {
                            ...
                            try
                            {
                                $this->ethernets[0] = new Interfc(Array(
                                    'name' => 'e0',
                                    'type' => 'ethernet'
                                ) , 0);
                            }
                        ...
                        else
                        {
                            ...
                            try
                            {
                                $this->ethernets[1] = new Interfc(Array(
                                    'name' => 'fa0',
                                    'type' => 'ethernet'
                                ) , 1);
                            }
            ...
            case 'qemu':
                $this->flags_eth = '';
                if (substr($this->getTemplate() , 0, 6) == "timos-" || substr($this->getTemplate() , 0, 9) == "timoscpm-" || substr($this->getTemplate() , 0, 9) == "timosiom-")
                {
                    $tmpl = strtok($this->getTemplate() , "-");
                }
                else
                {
                ...
                switch ($this->getTemplate())
                {
                    default:
                        ...
                        {
                            $format = EthFormat2val($p['eth_format']);
                        ...
                        //error_log(date('M d H:i:s ').$format['prefix'].",".$format['slotstart'].','.$format['sep'].','.$format['first'].','.$format['mod']);
                        $pass = 0;
                        for ($i = 0;$i < $this->ethernet;$i++)
                        {
                            $n = $prefix;
                            if (isset($p['eth_name'][$i]) && $p['eth_name'][$i] != '')
                            {
                                $n = $p['eth_name'][$i];
                                $pass += 1;
                            }
                        ...
                        for ($x = 0;$x < $this->serial;$x++)
                        {
                            for ($y = 0;$y <= 3;$y++)
                            {
                                ...
                                else
                                {
                                    ...
                                    try
                                    {
                                        $this->serials[$i] = new Interfc(Array(
                                            'name' => $n,
                                            'type' => 'serial'
                                        ) , $i);
                                    }
                            ...
                            switch ($s)
                            {
                                ...
                                case 'NM-1FE-TX':
                                    ...
                                    else
                                    {
                                        ...
                                        try
                                        {
                                            $this->ethernets[16 * $i] = new Interfc(Array(
                                                'name' => 'fa' . $i . '/0',
                                                'type' => 'ethernet'
                                            ) , 16 * $i);
                                        }
                                ...
                                case 'NM-16ESW':
                                    ...
                                    for ($p = 0;$p <= 15;$p++)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            try
                                            {
                                                $this->ethernets[16 * $i + $p] = new Interfc(Array(
                                                    'name' => 'fa' . $i . '/' . $p,
                                                    'type' => 'ethernet'
                                                ) , 16 * $i + $p);
                                            }
            ...
            public function unlinkInterface($i)
            {
                ...
                {
                    return $this->ethernets[$i]->edit(Array(
                        'network_id' => ''
                    ));
                }
                ...
                {
                    return $this->serials[$i]->edit(Array(
                        'remote_id' => '',
                        'remote_if' => ''
                    ));
                }
