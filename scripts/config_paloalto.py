...
def node_login(handler):
    ...
    if i == 0 or i == 1:
        ...
        try:
            j = handler.expect(['admin@.*>', 'Password:'], timeout = longtimeout)
        except:
            print('ERROR: error waiting for ["admin@.*>", "password:"] prompt.')
            node_quit(handler)
            return False
        ...
        if j == 0:
            ...
            try:
                handler.expect('admin@.*>', timeout = longtimeout)
            except:
                print('ERROR: error waiting for "admin@.*>" prompt.')
                node_quit(handler)
                return False
            return True
        else:
        ...
        try:
            handler.expect('admin@.*>', timeout = expctimeout)
        except:
            print('ERROR: error waiting for "admin@.*>" prompt.')
            node_quit(handler)
            return False
...
def config_get(handler):
    ...
    while True:
        try:
            handler.expect('admin@.*>', timeout = 0.1)
        except:
            break

    # set cli output format to set
    handler.sendline('set cli op-command-xml-output on')
    try:
        handler.expect('admin@.*>', timeout = longtimeout)
    except:
        print('ERROR: error waiting for "admin@.*>" prompt.')
    ...
    # Disable paging
    handler.sendline('set cli pager off')
    try:
        handler.expect('admin@.*>', timeout = longtimeout)
    except:
        print('ERROR: error waiting for "admin@.*>" prompt.')
    ...
    handler.sendline('set cli scripting-mode on')
    try:
        handler.expect('admin@.*>', timeout = longtimeout)
    except:
        print('ERROR: error waiting for "admin@.*>" prompt.')
    ...
    # Getting the config
    handler.sendline('show config running')
    try:
        handler.expect('</result></response>', timeout = longtimeout)
    except:
        print('ERROR: error waiting for the end of the configuration output.')
        node_quit(handler)
        return False
    ...
    config = re.sub('.* show config running\n+', '', config, flags=re.DOTALL)             # Remove CLI-commands before the config output
    config = re.sub('<response .*<result>', '<?xml version="1.0"?>\n', config, flags=re.DOTALL) # Remove invalid Header and add valid header
    config = re.sub('</result></response>', '', config, flags=re.DOTALL)                        # Remove invalid Footer in case it gets captured
