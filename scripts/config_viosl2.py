...
expctimeout = 15     # Maximum time for each short expect
...
def node_login(handler):
    ...
    if i == 0:
        ...
        handler.sendline('\r\n')
        try:
            handler.expect('Switch>', timeout = expctimeout)
        except:
...
def config_put(handler):
    while True:
        try:
           i = handler.expect(['CVAC-4-CONFIG_DONE','PLATFORM-5-SIGNATURE_VERIFIED'], timeout)
        except:
           return False
        return True
