    ...
    try:
        if action == 'put':
            ...
            if rc != True:
                ...
                sys.exit(1)
            # copy config file into docker
            p1 = subprocess.Popen(["/usr/bin/docker -H=tcp://127.0.0.1:4243 cp %s %s:/tmp/startup.sh" %(filename,docker_id)], stdout=subprocess.PIPE, shell=True)
            p1.wait()
            if p1.returncode != 0:
               print('ERROR: error opy startup script')
               sys.exit(1)
            # Execute script
            p1 = subprocess.Popen(["/usr/bin/docker -H=tcp://127.0.0.1:4243 exec -u root %s sh -e /tmp/startup.sh" %(docker_id)], stdout=subprocess.PIPE, shell=True)
            p1.wait()
            if p1.returncode != 0:
               print('ERROR: error executing startup script')
               sys.exit(1)
