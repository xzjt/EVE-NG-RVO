...
def config_get(handler):
    ...
    # Disable paging
    handler.sendline('set terminal length 0')
    try:
        handler.expect(':~\$', timeout = expctimeout)
    except:
        print('ERROR: error waiting for "#" prompt.')
    ...
    # Getting the config
    handler.sendline('show configuration commands')
    try:
    ...
    config = re.sub('.*show configuration commands\n', '', config, flags=re.DOTALL)          # Header
...
def config_put(handler, config):
    ...
    try:
        handler.expect('\[edit', timeout = expctimeout)
    except:
    ...
    try:
        handler.expect('Done', timeout = expctimeout)
    except:
        print('ERROR: error waiting for "Done" prompt.')
        node_quit(handler)
        return False
    handler.sendline('\r\n')
    try:
